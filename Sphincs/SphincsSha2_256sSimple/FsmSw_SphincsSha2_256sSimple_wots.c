...
/**********************************************************************************************************************/
static void fsmsw_sphincssha2_256ssimple_GenChain(uint8 *out, const uint8 *in, uint32 start, uint32 steps, 
                                                   const sphincs_sha2_256s_ctx *ctx, uint32 addr[8]);
static void fsmsw_sphincssha2_256ssimple_BaseW(uint32 *output, sint32 out_len, const uint8 *input);
static void fsmsw_sphincssha2_256ssimple_WotsChecksum(uint32 *csum_base_w, const uint32 *msg_base_w);

/**********************************************************************************************************************/
/* PRIVATE FUNCTIONS DEFINITIONS                                                                                      */
/**********************************************************************************************************************/
/***********************************************************************************************************************
* Name:        fsmsw_sphincssha2_256ssimple_GenChain
*
...
/***********************************************************************************************************************
* Name:        fsmsw_sphincssha2_256ssimple_BaseW
*
...
/***********************************************************************************************************************
* Name:        fsmsw_sphincssha2_256ssimple_WotsChecksum
*
...
{
    uint32 csum = 0;
    uint8 csum_bytes[((FSMSW_SPHINCSSHA2_256SSIMPLE_WOTS_LEN2 * FSMSW_SPHINCSSHA2_256SSIMPLE_WOTS_LOGW) + 7u) / 8u];
    uint32 i;
    ...
    csum = csum << ((8u - ((FSMSW_SPHINCSSHA2_256SSIMPLE_WOTS_LEN2 * FSMSW_SPHINCSSHA2_256SSIMPLE_WOTS_LOGW) % 8u))
                     % 8u);
    FsmSw_Sphincs_ull_to_bytes(csum_bytes, sizeof(csum_bytes), csum);
    fsmsw_sphincssha2_256ssimple_BaseW(csum_base_w, (sint32)FSMSW_SPHINCSSHA2_256SSIMPLE_WOTS_LEN2, csum_bytes);
}
...
/**********************************************************************************************************************/
/***********************************************************************************************************************
* Name:        FsmSw_SphincsSha2_256sSimple_chain_lengths
*
...
{
    fsmsw_sphincssha2_256ssimple_BaseW(lengths, (sint32)FSMSW_SPHINCSSHA2_256SSIMPLE_WOTS_LEN1, msg);
    fsmsw_sphincssha2_256ssimple_WotsChecksum(&lengths[FSMSW_SPHINCSSHA2_256SSIMPLE_WOTS_LEN1], lengths);
}

/***********************************************************************************************************************
* Name:        FsmSw_SphincsSha2_256sSimple_wots_pk_from_sig
*
...
void FsmSw_SphincsSha2_256sSimple_wots_pk_from_sig(uint8 *pk, const uint8 *sig, const uint8 *msg,
                                                   const sphincs_sha2_256s_ctx *ctx, uint32 addr[8])
{
    uint32 lengths[FSMSW_SPHINCSSHA2_256SSIMPLE_WOTS_LEN];
    uint32 i;

    FsmSw_SphincsSha2_256sSimple_chain_lengths(lengths, msg);

    for (i = 0; i < FSMSW_SPHINCSSHA2_256SSIMPLE_WOTS_LEN; i++)
    {
        FsmSw_SphincsSha2_set_chain_addr(addr, i);
        fsmsw_sphincssha2_256ssimple_GenChain(&pk[i * FSMSW_SPHINCSSHA2_256SSIMPLE_N], 
                                               &sig[i * FSMSW_SPHINCSSHA2_256SSIMPLE_N],
                                               lengths[i], FSMSW_SPHINCSSHA2_256SSIMPLE_WOTS_W - 1u - lengths[i], 
                                               ctx, addr);
    }
}
