...
#define SPX_INBLOCKS                (uint32)((uint32)((uint32)(((FSMSW_SPHINCSSHA2_256SSIMPLE_N + \
...
/**********************************************************************************************************************/
/***********************************************************************************************************************
* Name:        FsmSw_SphincsSha2_256sSimple_mgf1_256
*
...
***********************************************************************************************************************/
void FsmSw_SphincsSha2_256sSimple_mgf1_256(uint8 *out, uint32 outlen, const uint8 *in, uint32 inlen)
{
     uint8 inbuf[FSMSW_SPHINCSSHA2_256SSIMPLE_MGF1_256_BUF_LEN + 4u];
...
/***********************************************************************************************************************
* Name:        FsmSw_SphincsSha2_256sSimple_mgf1_512
*
...
{
    uint8 inbuf[FSMSW_SPHINCSSHA2_256SSIMPLE_MGF1_512_BUF_LEN + 4u];
...
/***********************************************************************************************************************
* Name:        FsmSw_SphincsSha2_256sSimple_prf_addr
*
...
*
***********************************************************************************************************************/
void FsmSw_SphincsSha2_256sSimple_prf_addr(uint8 *out, const sphincs_sha2_256s_ctx *ctx, const uint32 addr[8])
{
    sha256ctx sha2_state;
    uint8 buf[SPX_SHA256_ADDR_BYTES + FSMSW_SPHINCSSHA2_256SSIMPLE_N];
    uint8 outbuf[FSMSW_SPHINCS_SHA256_OUTPUT_BYTES];
    ...
    FsmSw_sha256_inc_finalize(outbuf, &sha2_state, buf, SPX_SHA256_ADDR_BYTES + FSMSW_SPHINCSSHA2_256SSIMPLE_N);

    FsmSw_CommonLib_memcpy(out, outbuf, FSMSW_SPHINCSSHA2_256SSIMPLE_N);
}

/***********************************************************************************************************************
* Name:        FsmSw_SphincsSha2_256sSimple_gen_message_random
*
...
*
***********************************************************************************************************************/
void FsmSw_SphincsSha2_256sSimple_gen_message_random(uint8 *R, const uint8 *sk_prf, const uint8 *optrand,
                                                     const uint8 *m, uint32 mlen, const sphincs_sha2_256s_ctx *ctx)
{
    (void)ctx;
    ...
    if (FSMSW_SPHINCSSHA2_256SSIMPLE_N + mlen < SPX_SHAX_BLOCK_BYTES)
    {
        FsmSw_CommonLib_memcpy(&buf[FSMSW_SPHINCSSHA2_256SSIMPLE_N], m, mlen);
        shaX_inc_finalize(&buf[SPX_SHAX_BLOCK_BYTES], &state,
                          buf, mlen + FSMSW_SPHINCSSHA2_256SSIMPLE_N);
    }
    /* Otherwise first fill a block, so that finalize only uses the message */
    else
    {
        FsmSw_CommonLib_memcpy(&buf[FSMSW_SPHINCSSHA2_256SSIMPLE_N], m,
                               SPX_SHAX_BLOCK_BYTES - FSMSW_SPHINCSSHA2_256SSIMPLE_N);
        shaX_inc_blocks(&state, buf, 1u);

        m = &m[SPX_SHAX_BLOCK_BYTES - FSMSW_SPHINCSSHA2_256SSIMPLE_N];
        mlen -= SPX_SHAX_BLOCK_BYTES - FSMSW_SPHINCSSHA2_256SSIMPLE_N;
...
/***********************************************************************************************************************
* Name:        FsmSw_SphincsSha2_256sSimple_hash_message
*
...
*
***********************************************************************************************************************/
void FsmSw_SphincsSha2_256sSimple_hash_message(uint8 *digest, uint64 *tree, uint32 *leaf_idx, const uint8 *R,
                                               ...
                                               const sphincs_sha2_256s_ctx *ctx)
{
    ...
    uint8 seed[2u * FSMSW_SPHINCSSHA2_256SSIMPLE_N + SPX_SHAX_OUTPUT_BYTES];
    ...
    if (FSMSW_SPHINCSSHA2_256SSIMPLE_N + FSMSW_SPHINCSSHA2_256SSIMPLE_PK_BYTES + mlen <
        SPX_INBLOCKS * SPX_SHAX_BLOCK_BYTES)
    {
        FsmSw_CommonLib_memcpy(&inbuf[FSMSW_SPHINCSSHA2_256SSIMPLE_N + FSMSW_SPHINCSSHA2_256SSIMPLE_PK_BYTES], m, mlen);
        shaX_inc_finalize(&seed[2u * FSMSW_SPHINCSSHA2_256SSIMPLE_N], &state, inbuf,
                                FSMSW_SPHINCSSHA2_256SSIMPLE_N + FSMSW_SPHINCSSHA2_256SSIMPLE_PK_BYTES + mlen);
    }
    /* Otherwise first fill a block, so that finalize only uses the message */
    else
    {
        FsmSw_CommonLib_memcpy(&inbuf[FSMSW_SPHINCSSHA2_256SSIMPLE_N + FSMSW_SPHINCSSHA2_256SSIMPLE_PK_BYTES], m,
                               SPX_INBLOCKS * SPX_SHAX_BLOCK_BYTES - FSMSW_SPHINCSSHA2_256SSIMPLE_N -
                               FSMSW_SPHINCSSHA2_256SSIMPLE_PK_BYTES);
        shaX_inc_blocks(&state, inbuf, SPX_INBLOCKS);

        m = &m[SPX_INBLOCKS * SPX_SHAX_BLOCK_BYTES - FSMSW_SPHINCSSHA2_256SSIMPLE_N -
               FSMSW_SPHINCSSHA2_256SSIMPLE_PK_BYTES];
        mlen -= SPX_INBLOCKS * SPX_SHAX_BLOCK_BYTES - FSMSW_SPHINCSSHA2_256SSIMPLE_N -
                FSMSW_SPHINCSSHA2_256SSIMPLE_PK_BYTES;
        shaX_inc_finalize(&seed[2u * FSMSW_SPHINCSSHA2_256SSIMPLE_N], &state, m, (uint32)mlen);
    }
