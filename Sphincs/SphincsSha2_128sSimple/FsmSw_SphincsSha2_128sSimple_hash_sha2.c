                                                     ...
                                                     const uint8 *m, uint32 mlen, const sphincs_sha2_128s_ctx *ctx)
{
    ...
    if ((FSMSW_SPHINCSSHA2_128SSIMPLE_N + mlen_temp) < SPX_SHAX_BLOCK_BYTES)
    {
        ...
        shaX_inc_finalize(&buf[SPX_SHAX_BLOCK_BYTES], &state,
                          buf, mlen_temp + FSMSW_SPHINCSSHA2_128SSIMPLE_N);
    }
                                               ...
                                               const sphincs_sha2_128s_ctx *ctx)
{
    ...
    if ((FSMSW_SPHINCSSHA2_128SSIMPLE_N + FSMSW_SPHINCSSHA2_128SSIMPLE_PK_BYTES + 
		mlen_temp) < (SPX_INBLOCKS * SPX_SHAX_BLOCK_BYTES))
    {
        FsmSw_CommonLib_memcpy(&inbuf[FSMSW_SPHINCSSHA2_128SSIMPLE_N + FSMSW_SPHINCSSHA2_128SSIMPLE_PK_BYTES], 
        m_temp, mlen_temp);
    ...
    else
    {
        ...
        /* polyspace +2 MISRA2012:2.2 [Justified:]"Calculation is important if defines should change 
        and therefore not dead code" */ 
        mlen_temp -= (SPX_INBLOCKS * SPX_SHAX_BLOCK_BYTES) - FSMSW_SPHINCSSHA2_128SSIMPLE_N -
                                                      FSMSW_SPHINCSSHA2_128SSIMPLE_PK_BYTES;
