...
/**********************************************************************************************************************/
/***********************************************************************************************************************
* Name:        FsmSw_SphincsSha2_128sSimple_compute_root
*
...
*
***********************************************************************************************************************/
void FsmSw_SphincsSha2_128sSimple_compute_root(uint8 *root, const uint8 *leaf, uint32 leaf_idx, uint32 idx_offset,
                                               const uint8 *auth_path, uint32 tree_height,
                                               const sphincs_sha2_128s_ctx *ctx, uint32 addr[8])
{
    uint32 i;
    uint8 buffer[2u * FSMSW_SPHINCSSHA2_128SSIMPLE_N];
    ...
    if (0u < (leaf_idx & 1u))
    {
        ...
        FsmSw_CommonLib_memcpy(buffer, auth_path, FSMSW_SPHINCSSHA2_128SSIMPLE_N);
    }
    else
    {
        FsmSw_CommonLib_memcpy(buffer, leaf, FSMSW_SPHINCSSHA2_128SSIMPLE_N);
        FsmSw_CommonLib_memcpy(&buffer[FSMSW_SPHINCSSHA2_128SSIMPLE_N], auth_path, FSMSW_SPHINCSSHA2_128SSIMPLE_N);
    }
    auth_path = &auth_path[FSMSW_SPHINCSSHA2_128SSIMPLE_N];
...
/***********************************************************************************************************************
* Name:        FsmSw_SphincsSha2_128sSimple_treehash
*
...
void FsmSw_SphincsSha2_128sSimple_treehash(uint8 *root, uint8 *auth_path, const sphincs_sha2_128s_ctx *ctx,
                                           ...
                                           uint32 tree_addr[8])
{
    uint8  stack[(FSMSW_SPHINCSSHA2_128SSIMPLE_TREEHASH_BUF_LEN + 1u)*FSMSW_SPHINCSSHA2_128SSIMPLE_N];
    uint32 heights[FSMSW_SPHINCSSHA2_128SSIMPLE_TREEHASH_BUF_LEN + 1u];
    uint32 offset = 0;
    uint32 idx;
    uint32 tree_idx;
        ...
        while (offset >= 2u && heights[offset - 1u] == heights[offset - 2u])
        {
            ...
            if (((leaf_idx >> heights[offset - 1u]) ^ 0x1u) == tree_idx)
            {
                FsmSw_CommonLib_memcpy(&auth_path[heights[offset - 1u]*FSMSW_SPHINCSSHA2_128SSIMPLE_N],
                                       &stack[(offset - 1u)*FSMSW_SPHINCSSHA2_128SSIMPLE_N],
                                       FSMSW_SPHINCSSHA2_128SSIMPLE_N);
            }
        }
    }
