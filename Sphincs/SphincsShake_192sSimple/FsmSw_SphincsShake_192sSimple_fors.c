...
/**********************************************************************************************************************/
static void fors_gen_sk(uint8 *sk, const sphincs_shake_192s_ctx *ctx, const uint32 fors_leaf_addr[8]);
static void fors_sk_to_leaf(uint8 *leaf, const uint8 *sk, const sphincs_shake_192s_ctx *ctx, 
                                            ...
                                            const uint32 fors_addr[8])
{
    ...
    for (i = 0; i < FSMSW_SPHINCSSHAKE_192SSIMPLE_FORS_TREES; i++)
    {
        ...
        FsmSw_SphincsShake_192sSimple_treehashx1(&roots[i * FSMSW_SPHINCSSHAKE_192SSIMPLE_N], sig_temp, ctx,
                                                    ...
                                                    const sphincs_shake_192s_ctx *ctx, const uint32 fors_addr[8])
{
    ...
    for (i = 0; i < FSMSW_SPHINCSSHAKE_192SSIMPLE_FORS_TREES; i++)
    {
        ...
        FsmSw_SphincsShake_192sSimple_compute_root(&roots[i * FSMSW_SPHINCSSHAKE_192SSIMPLE_N], leaf, indices[i],
		                                               idx_offset, sig_temp, FSMSW_SPHINCSSHAKE_192SSIMPLE_FORS_HEIGHT, ctx,
                                                   fors_tree_addr);
        sig_temp = &sig_temp[FSMSW_SPHINCSSHAKE_192SSIMPLE_N * FSMSW_SPHINCSSHAKE_192SSIMPLE_FORS_HEIGHT];
    }
