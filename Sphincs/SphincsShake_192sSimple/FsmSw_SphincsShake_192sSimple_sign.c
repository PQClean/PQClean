                                                        ...
                                                        const uint8 *sk)
{
    ...
    FsmSw_SphincsShake_192sSimple_gen_message_random(sig, sk_prf, optrand, m, mlen, &ctx);

    /* Derive the message digest and leaf index from R, PK and M. */
    FsmSw_SphincsShake_192sSimple_hash_message(mhash, &tree, &idx_leaf, sig, pk, m, mlen, &ctx);
    ...
    FsmSw_SphincsShake_192sSimple_fors_sign(sig, root, mhash, &ctx, wots_addr);
    sig = &sig[FSMSW_SPHINCSSHAKE_192SSIMPLE_FORS_BYTES];
    ...
    for (i = 0; i < FSMSW_SPHINCSSHAKE_192SSIMPLE_D; i++)
    {
        ...
        FsmSw_SphincsShake_192sSimple_merkle_sign(sig, root, &ctx, wots_addr, tree_addr, idx_leaf);
        sig = &sig[FSMSW_SPHINCSSHAKE_192SSIMPLE_WOTS_BYTES + FSMSW_SPHINCSSHAKE_192SSIMPLE_TREE_HEIGHT *
                                                     ...
                                                     const uint8 *pk)
{
    ...
    FsmSw_SphincsShake_192sSimple_fors_pk_from_sig(root, sig, mhash, &ctx, wots_addr);
    sig = &sig[FSMSW_SPHINCSSHAKE_192SSIMPLE_FORS_BYTES];
    ...
    for (i = 0; i < FSMSW_SPHINCSSHAKE_192SSIMPLE_D; i++)
    {
        ...
        FsmSw_SphincsShake_192sSimple_wots_pk_from_sig(wots_pk, sig, root, &ctx, wots_addr);
        sig = &sig[FSMSW_SPHINCSSHAKE_192SSIMPLE_WOTS_BYTES];

        /* Compute the leaf node using the WOTS public key. */
        FsmSw_SphincsShake_192sSimple_thash(leaf, wots_pk, FSMSW_SPHINCSSHAKE_192SSIMPLE_WOTS_LEN, &ctx, wots_pk_addr);

        /* Compute the root node of this subtree. */
        FsmSw_SphincsShake_192sSimple_compute_root(root, leaf, idx_leaf, 0, sig,
                                                   FSMSW_SPHINCSSHAKE_192SSIMPLE_TREE_HEIGHT, &ctx, tree_addr);
        sig = &sig[FSMSW_SPHINCSSHAKE_192SSIMPLE_TREE_HEIGHT * FSMSW_SPHINCSSHAKE_192SSIMPLE_N];
