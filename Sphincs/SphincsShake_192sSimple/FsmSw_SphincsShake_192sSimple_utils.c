...
/**********************************************************************************************************************/
/***********************************************************************************************************************
* Name:        FsmSw_SphincsShake_192sSimple_compute_root
*
...
{
    uint32 i;
    uint8 buffer[2u * FSMSW_SPHINCSSHAKE_192SSIMPLE_N];
    ...
    if (0u < (leaf_idx & 1u))
    {
        ...
        FsmSw_CommonLib_memcpy(buffer, auth_path, FSMSW_SPHINCSSHAKE_192SSIMPLE_N);
    }
    else
    {
        FsmSw_CommonLib_memcpy(buffer, leaf, FSMSW_SPHINCSSHAKE_192SSIMPLE_N);
        FsmSw_CommonLib_memcpy(&buffer[FSMSW_SPHINCSSHAKE_192SSIMPLE_N], auth_path, FSMSW_SPHINCSSHAKE_192SSIMPLE_N);
    }
    auth_path = &auth_path[FSMSW_SPHINCSSHAKE_192SSIMPLE_N];
    ...
    for (i = 0; i < tree_height - 1u; i++)
    {
        ...
        else
        {
            ...
            FsmSw_CommonLib_memcpy(&buffer[FSMSW_SPHINCSSHAKE_192SSIMPLE_N], auth_path,
...
void FsmSw_SphincsShake_192sSimple_treehash(uint8 *root, uint8 *auth_path, const sphincs_shake_192s_ctx *ctx,
                                            ...
                                            uint32 tree_addr[8])
{
    uint8  stack[(FSMSW_SPHINCSSHAKE_192SSIMPLE_TREEHASH_BUF_LEN + 1u)*FSMSW_SPHINCSSHAKE_192SSIMPLE_N];
    uint32 heights[FSMSW_SPHINCSSHAKE_192SSIMPLE_TREEHASH_BUF_LEN + 1u];
    uint32 offset = 0;
    uint32 idx;
    uint32 tree_idx;
