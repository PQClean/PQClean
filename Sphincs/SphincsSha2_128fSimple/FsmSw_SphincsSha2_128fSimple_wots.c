...
/**********************************************************************************************************************/
static void gen_chain(uint8 *out, const uint8 *in, uint32 start, uint32 steps, const sphincs_sha2_128f_ctx *ctx,
                      uint32 addr[8]);
...
static void wots_checksum(uint32 *csum_base_w, const uint32 *msg_base_w)
{
    ...
    base_w(csum_base_w, (sint32)FSMSW_SPHINCSSHA2_128FSIMPLE_WOTS_LEN2, csum_bytes);
}
...
{
    base_w(lengths, (sint32)FSMSW_SPHINCSSHA2_128FSIMPLE_WOTS_LEN1, msg);
    wots_checksum(&lengths[FSMSW_SPHINCSSHA2_128FSIMPLE_WOTS_LEN1], lengths);
}
                                                   ...
                                                   const sphincs_sha2_128f_ctx *ctx, uint32 addr[8])
{
    ...
    for (i = 0; i < FSMSW_SPHINCSSHA2_128FSIMPLE_WOTS_LEN; i++)
    {
        ...
        gen_chain(&pk[i * FSMSW_SPHINCSSHA2_128FSIMPLE_N], &sig[i * FSMSW_SPHINCSSHA2_128FSIMPLE_N],
                  lengths[i], FSMSW_SPHINCSSHA2_128FSIMPLE_WOTS_W - 1u - lengths[i], ctx, addr);
    }
}
