...
/**********************************************************************************************************************/
/***********************************************************************************************************************
* Name:        FsmSw_SphincsShake_256sSimple_compute_root
*
...
{
    uint32 i;
    uint8 buffer[2u * FSMSW_SPHINCSSHAKE_256SSIMPLE_N];
    ...
    if (0u < (leaf_idx & 1u))
    {
        ...
        FsmSw_CommonLib_memcpy(buffer, auth_path, FSMSW_SPHINCSSHAKE_256SSIMPLE_N);
    }
    else
    {
        FsmSw_CommonLib_memcpy(buffer, leaf, FSMSW_SPHINCSSHAKE_256SSIMPLE_N);
        FsmSw_CommonLib_memcpy(&buffer[FSMSW_SPHINCSSHAKE_256SSIMPLE_N], auth_path, FSMSW_SPHINCSSHAKE_256SSIMPLE_N);
    }
    auth_path = &auth_path[FSMSW_SPHINCSSHAKE_256SSIMPLE_N];
    ...
    for (i = 0; i < tree_height - 1u; i++)
    {
        ...
        else
        {
            ...
            FsmSw_CommonLib_memcpy(&buffer[FSMSW_SPHINCSSHAKE_256SSIMPLE_N], auth_path,
...
/***********************************************************************************************************************
* Name:        FsmSw_SphincsShake_256sSimple_treehash
*
...
void FsmSw_SphincsShake_256sSimple_treehash(uint8 *root, uint8 *auth_path, const sphincs_shake_256s_ctx *ctx,
                                            ...
                                            uint32 tree_addr[8])
{
    uint8  stack[(FSMSW_SPHINCSSHAKE_256SSIMPLE_TREEHASH_BUF_LEN + 1u)*FSMSW_SPHINCSSHAKE_256SSIMPLE_N];
    uint32 heights[FSMSW_SPHINCSSHAKE_256SSIMPLE_TREEHASH_BUF_LEN + 1u];
    uint32 offset = 0;
    uint32 idx;
    uint32 tree_idx;
    ...
    for (idx = 0; idx < (uint32)((uint32)1u << tree_height); idx++)
    {
        ...
        while (offset >= 2u && heights[offset - 1u] == heights[offset - 2u])
        {
            ...
            FsmSw_SphincsShake_256sSimple_thash(&stack[(offset - 2u)*FSMSW_SPHINCSSHAKE_256SSIMPLE_N],
                                                &stack[(offset - 2u)*FSMSW_SPHINCSSHAKE_256SSIMPLE_N],
                                                2u, ctx, tree_addr);
            offset--;
