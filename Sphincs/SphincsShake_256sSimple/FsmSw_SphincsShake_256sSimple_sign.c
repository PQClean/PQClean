...
/**********************************************************************************************************************/
/***********************************************************************************************************************
* Name:        FsmSw_SphincsShake_256sSimple_crypto_sign_seed_keypair
*
...
*
***********************************************************************************************************************/
int FsmSw_SphincsShake_256sSimple_crypto_sign_seed_keypair(uint8 *pk, uint8 *sk, const uint8 *seed)
{
    sphincs_shake_256s_ctx ctx;
    ...
    FsmSw_SphincsShake_256sSimple_initialize_hash_function(&ctx);

    /* Compute root node of the top-most subtree. */
    FsmSw_SphincsShake_256sSimple_merkle_gen_root(&sk[3u * FSMSW_SPHINCSSHAKE_256SSIMPLE_N], &ctx);

    /* cleanup */
    FsmSw_SphincsShake_256sSimple_free_hash_function(&ctx);

    FsmSw_CommonLib_memcpy(&pk[FSMSW_SPHINCSSHAKE_256SSIMPLE_N],
                           &sk[3u * FSMSW_SPHINCSSHAKE_256SSIMPLE_N], FSMSW_SPHINCSSHAKE_256SSIMPLE_N);
...
*
***********************************************************************************************************************/
int FsmSw_SphincsShake_256sSimple_crypto_sign_keypair(uint8 *pk, uint8 *sk)
{
    uint8 seed[FSMSW_SPHINCSSHAKE_256SSIMPLE_CRYPTO_SEEDBYTES];
    (void)FsmSw_CommonLib_randombytes(seed, FSMSW_SPHINCSSHAKE_256SSIMPLE_CRYPTO_SEEDBYTES);
    (void)FsmSw_SphincsShake_256sSimple_crypto_sign_seed_keypair(pk, sk, seed);
...
*
***********************************************************************************************************************/
int FsmSw_SphincsShake_256sSimple_crypto_sign_signature(uint8 *sig, uint32 *siglen, const uint8 *m, uint32 mlen,
                                                        const uint8 *sk)
{
    sphincs_shake_256s_ctx ctx;
    ...
    uint8 optrand[FSMSW_SPHINCSSHAKE_256SSIMPLE_N];
    uint8 mhash[FSMSW_SPHINCSSHAKE_256SSIMPLE_FORS_MSG_BYTES];
    uint8 root[FSMSW_SPHINCSSHAKE_256SSIMPLE_N];
    uint32 i;
    uint64 tree;
    uint32 idx_leaf;
    ...
    FsmSw_SphincsShake_256sSimple_gen_message_random(sig, sk_prf, optrand, m, mlen, &ctx);

    /* Derive the message digest and leaf index from R, PK and M. */
    FsmSw_SphincsShake_256sSimple_hash_message(mhash, &tree, &idx_leaf, sig, pk, m, mlen, &ctx);
    sig = &sig[FSMSW_SPHINCSSHAKE_256SSIMPLE_N];

    FsmSw_SphincsShake_set_tree_addr(wots_addr, tree);
    FsmSw_SphincsShake_256sSimple_set_keypair_addr(wots_addr, idx_leaf);

    /* Sign the message hash using FORS. */
    FsmSw_SphincsShake_256sSimple_fors_sign(sig, root, mhash, &ctx, wots_addr);
    sig = &sig[FSMSW_SPHINCSSHAKE_256SSIMPLE_FORS_BYTES];
    ...
    for (i = 0; i < FSMSW_SPHINCSSHAKE_256SSIMPLE_D; i++)
    {
        ...
        FsmSw_SphincsShake_256sSimple_merkle_sign(sig, root, &ctx, wots_addr, tree_addr, idx_leaf);
        sig = &sig[FSMSW_SPHINCSSHAKE_256SSIMPLE_WOTS_BYTES + FSMSW_SPHINCSSHAKE_256SSIMPLE_TREE_HEIGHT *
...
*
***********************************************************************************************************************/
int FsmSw_SphincsShake_256sSimple_crypto_sign_verify(const uint8 *sig, uint32 siglen, const uint8 *m, uint32 mlen,
                                                     const uint8 *pk)
{
    sphincs_shake_256s_ctx ctx;
    const uint8 *pub_root = &pk[FSMSW_SPHINCSSHAKE_256SSIMPLE_N];
    uint8 mhash[FSMSW_SPHINCSSHAKE_256SSIMPLE_FORS_MSG_BYTES];
    uint8 wots_pk[FSMSW_SPHINCSSHAKE_256SSIMPLE_WOTS_BYTES];
    uint8 root[FSMSW_SPHINCSSHAKE_256SSIMPLE_N];
    uint8 leaf[FSMSW_SPHINCSSHAKE_256SSIMPLE_N];
    uint32 i;
    uint64 tree;
    uint32 idx_leaf;
    uint32 wots_addr[8] = {0};
    ...
    * The additional FSMSW_SPHINCSSHAKE_256SSIMPLE_N is a result of the hash domain separator. */
    ...
    FsmSw_SphincsShake_256sSimple_fors_pk_from_sig(root, sig, mhash, &ctx, wots_addr);
    sig = &sig[FSMSW_SPHINCSSHAKE_256SSIMPLE_FORS_BYTES];
    ...
    for (i = 0; i < FSMSW_SPHINCSSHAKE_256SSIMPLE_D; i++)
    {
        ...
        FsmSw_SphincsShake_256sSimple_wots_pk_from_sig(wots_pk, sig, root, &ctx, wots_addr);
        sig = &sig[FSMSW_SPHINCSSHAKE_256SSIMPLE_WOTS_BYTES];

        /* Compute the leaf node using the WOTS public key. */
        FsmSw_SphincsShake_256sSimple_thash(leaf, wots_pk, FSMSW_SPHINCSSHAKE_256SSIMPLE_WOTS_LEN, &ctx, wots_pk_addr);

        /* Compute the root node of this subtree. */
        FsmSw_SphincsShake_256sSimple_compute_root(root, leaf, idx_leaf, 0, sig,
                                                   FSMSW_SPHINCSSHAKE_256SSIMPLE_TREE_HEIGHT, &ctx, tree_addr);
        sig = &sig[FSMSW_SPHINCSSHAKE_256SSIMPLE_TREE_HEIGHT * FSMSW_SPHINCSSHAKE_256SSIMPLE_N];
...
*
***********************************************************************************************************************/
int FsmSw_SphincsShake_256sSimple_crypto_sign(uint8 *sm, uint32 *smlen, const uint8 *m, uint32 mlen, const uint8 *sk)
{
    uint32 siglen;
...
*
***********************************************************************************************************************/
int FsmSw_SphincsShake_256sSimple_crypto_sign_open(uint8 *m, uint32 *mlen, const uint8 *sm, uint32 smlen,
                                                   const uint8 *pk)
{
