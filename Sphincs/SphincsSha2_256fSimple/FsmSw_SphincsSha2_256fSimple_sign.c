...
/**********************************************************************************************************************/
/***********************************************************************************************************************
* Name:        FsmSw_SphincsSha2_256fSimple_crypto_sign_seed_keypair
*
...
*
***********************************************************************************************************************/
int FsmSw_SphincsSha2_256fSimple_crypto_sign_seed_keypair(uint8 *pk, uint8 *sk, const uint8 *seed)
{
    sphincs_sha2_256f_ctx ctx;
    ...
    FsmSw_SphincsSha2_256fSimple_initialize_hash_function(&ctx);

    /* Compute root node of the top-most subtree. */
    FsmSw_SphincsSha2_256fSimple_merkle_gen_root(&sk[3u * FSMSW_SPHINCSSHA2_256FSIMPLE_N], &ctx);

    FsmSw_CommonLib_memcpy(&pk[FSMSW_SPHINCSSHA2_256FSIMPLE_N],
                           &sk[3u * FSMSW_SPHINCSSHA2_256FSIMPLE_N], FSMSW_SPHINCSSHA2_256FSIMPLE_N);
...
*
***********************************************************************************************************************/
int FsmSw_SphincsSha2_256fSimple_crypto_sign_keypair(uint8 *pk, uint8 *sk)
{
    uint8 seed[FSMSW_SPHINCSSHA2_256FSIMPLE_CRYPTO_SEEDBYTES];
    (void)FsmSw_CommonLib_randombytes(seed, FSMSW_SPHINCSSHA2_256FSIMPLE_CRYPTO_SEEDBYTES);
    (void)FsmSw_SphincsSha2_256fSimple_crypto_sign_seed_keypair(pk, sk, seed);
...
*
***********************************************************************************************************************/
int FsmSw_SphincsSha2_256fSimple_crypto_sign_signature(uint8 *sig, uint32 *siglen, const uint8 *m, uint32 mlen,
                                                       const uint8 *sk)
{
    sphincs_sha2_256f_ctx ctx;
    ...
    uint8 optrand[FSMSW_SPHINCSSHA2_256FSIMPLE_N];
    uint8 mhash[FSMSW_SPHINCSSHA2_256FSIMPLE_FORS_MSG_BYTES];
    uint8 root[FSMSW_SPHINCSSHA2_256FSIMPLE_N];
    uint32 i;
    uint64 tree;
    uint32 idx_leaf;
    ...
    FsmSw_SphincsSha2_256fSimple_gen_message_random(sig, sk_prf, optrand, m, mlen, &ctx);

    /* Derive the message digest and leaf index from R, PK and M. */
    FsmSw_SphincsSha2_256fSimple_hash_message(mhash, &tree, &idx_leaf, sig, pk, m, mlen, &ctx);
    sig = &sig[FSMSW_SPHINCSSHA2_256FSIMPLE_N];

    FsmSw_SphincsSha2_set_tree_addr(wots_addr, tree);
    FsmSw_SphincsSha2_256fSimple_set_keypair_addr(wots_addr, idx_leaf);

    /* Sign the message hash using FORS. */
    FsmSw_SphincsSha2_256fSimple_fors_sign(sig, root, mhash, &ctx, wots_addr);
    sig = &sig[FSMSW_SPHINCSSHA2_256FSIMPLE_FORS_BYTES];
    ...
    for (i = 0; i < FSMSW_SPHINCSSHA2_256FSIMPLE_D; i++)
    {
        ...
        FsmSw_SphincsSha2_256fSimple_merkle_sign(sig, root, &ctx, wots_addr, tree_addr, idx_leaf);
        sig = &sig[FSMSW_SPHINCSSHA2_256FSIMPLE_WOTS_BYTES + FSMSW_SPHINCSSHA2_256FSIMPLE_TREE_HEIGHT *
...
*
***********************************************************************************************************************/
int FsmSw_SphincsSha2_256fSimple_crypto_sign_verify(const uint8 *sig, uint32 siglen, const uint8 *m, uint32 mlen,
                                                    const uint8 *pk)
{
    sphincs_sha2_256f_ctx ctx;
    const uint8 *pub_root = &pk[FSMSW_SPHINCSSHA2_256FSIMPLE_N];
    uint8 mhash[FSMSW_SPHINCSSHA2_256FSIMPLE_FORS_MSG_BYTES];
    uint8 wots_pk[FSMSW_SPHINCSSHA2_256FSIMPLE_WOTS_BYTES];
    uint8 root[FSMSW_SPHINCSSHA2_256FSIMPLE_N];
    ...
    FsmSw_SphincsSha2_256fSimple_fors_pk_from_sig(root, sig, mhash, &ctx, wots_addr);
    sig = &sig[FSMSW_SPHINCSSHA2_256FSIMPLE_FORS_BYTES];
    ...
    for (i = 0; i < FSMSW_SPHINCSSHA2_256FSIMPLE_D; i++)
    {
        ...
        FsmSw_SphincsSha2_256fSimple_wots_pk_from_sig(wots_pk, sig, root, &ctx, wots_addr);
        sig = &sig[FSMSW_SPHINCSSHA2_256FSIMPLE_WOTS_BYTES];

        /* Compute the leaf node using the WOTS public key. */
        FsmSw_SphincsSha2_256fSimple_thash(leaf, wots_pk, FSMSW_SPHINCSSHA2_256FSIMPLE_WOTS_LEN, &ctx, wots_pk_addr);

        /* Compute the root node of this subtree. */
        FsmSw_SphincsSha2_256fSimple_compute_root(root, leaf, idx_leaf, 0, sig,
                                                  FSMSW_SPHINCSSHA2_256FSIMPLE_TREE_HEIGHT, &ctx, tree_addr);
        sig = &sig[FSMSW_SPHINCSSHA2_256FSIMPLE_TREE_HEIGHT * FSMSW_SPHINCSSHA2_256FSIMPLE_N];
...
*
***********************************************************************************************************************/
int FsmSw_SphincsSha2_256fSimple_crypto_sign(uint8 *sm, uint32 *smlen, const uint8 *m, uint32 mlen, const uint8 *sk)
{
    uint32 siglen;
...
*
***********************************************************************************************************************/
int FsmSw_SphincsSha2_256fSimple_crypto_sign_open(uint8 *m, uint32 *mlen, const uint8 *sm, uint32 smlen,
                                                  const uint8 *pk)
{
