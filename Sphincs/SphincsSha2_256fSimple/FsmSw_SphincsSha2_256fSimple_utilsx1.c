...
{
    uint8 stack[FSMSW_SPHINCSSHA2_256FSIMPLE_TREEHASHX1_BUF_LEN * FSMSW_SPHINCSSHA2_256FSIMPLE_N];
    ...
    for (idx = 0; idx < 0xFFFFFFFFu; idx++)
    {
        ...
        for (h = 0;  h < 0xFFFFFFFFu; h++)
        {
            ...
            {
                FsmSw_CommonLib_memcpy( &auth_path[h * FSMSW_SPHINCSSHA2_256FSIMPLE_N],
                        &current[FSMSW_SPHINCSSHA2_256FSIMPLE_N],
            ...
            FsmSw_SphincsSha2_256fSimple_thash(&current[1u * FSMSW_SPHINCSSHA2_256FSIMPLE_N],
