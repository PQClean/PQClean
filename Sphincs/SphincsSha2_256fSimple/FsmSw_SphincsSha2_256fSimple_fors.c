...
/**********************************************************************************************************************/
static void fsmsw_sphincssha2_256fsimple_ForsGenSk(uint8 *sk, const sphincs_sha2_256f_ctx *ctx, 
                                                     const uint32 fors_leaf_addr[8]);
static void fsmsw_sphincssha2_256fsimple_ForsSkToLeaf(uint8 *leaf, const uint8 *sk, 
                                                         const sphincs_sha2_256f_ctx *ctx, 
                                                         const uint32 fors_leaf_addr[8]);
static void fsmsw_sphincssha2_256fsimple_ForsGenLeafx1(uint8 *leaf, const sphincs_sha2_256f_ctx *ctx, 
                                                         uint32 addr_idx, void *info);
static void fsmsw_sphincssha2_256fsimple_MessageToIndices(uint32 *indices, const uint8 *m);

/**********************************************************************************************************************/
/* PRIVATE FUNCTIONS DEFINITIONS                                                                                      */
/**********************************************************************************************************************/
/***********************************************************************************************************************
* Name:        fsmsw_sphincssha2_256fsimple_ForsGenSk
*
* Description: t.b.d
*
...
static void fsmsw_sphincssha2_256fsimple_ForsGenSk(uint8 *sk, const sphincs_sha2_256f_ctx *ctx, 
                                                     const uint32 fors_leaf_addr[8])
{
    FsmSw_SphincsSha2_256fSimple_prf_addr(sk, ctx, fors_leaf_addr);
}

/***********************************************************************************************************************
* Name:        fsmsw_sphincssha2_256fsimple_ForsSkToLeaf
*
* Description: t.b.d
*
...
/***********************************************************************************************************************
* Name:        fsmsw_sphincssha2_256fsimple_ForsGenLeafx1
*
* Description: t.b.d
*
* Arguments:   -       uint8                 *leaf:     t.b.d.
                                                         ...
                                                         uint32 addr_idx, void *info)
{
    ...
    FsmSw_SphincsSha2_set_tree_index(fors_leaf_addr, addr_idx);
    FsmSw_SphincsSha2_set_type(fors_leaf_addr, FSMSW_SPHINCS_ADDR_TYPE_FORSPRF);
    fsmsw_sphincssha2_256fsimple_ForsGenSk(leaf, ctx, fors_leaf_addr);

    FsmSw_SphincsSha2_set_type(fors_leaf_addr, FSMSW_SPHINCS_ADDR_TYPE_FORSTREE);
    fsmsw_sphincssha2_256fsimple_ForsSkToLeaf(leaf, leaf, ctx, fors_leaf_addr);
}

/***********************************************************************************************************************
* Name:        fsmsw_sphincssha2_256fsimple_MessageToIndices
*
* Description: Interprets m as FSMSW_SPHINCSSHA2_256FSIMPLE_FORS_HEIGHT-bit uint32. Assumes m contains at least
...
/**********************************************************************************************************************/
/***********************************************************************************************************************
* Name:        FsmSw_SphincsSha2_256fSimple_fors_sign
*
...
{
    uint32 indices[FSMSW_SPHINCSSHA2_256FSIMPLE_FORS_TREES];
    uint8 roots[FSMSW_SPHINCSSHA2_256FSIMPLE_FORS_TREES * FSMSW_SPHINCSSHA2_256FSIMPLE_N];
    ...
    FsmSw_SphincsSha2_set_type(fors_pk_addr, FSMSW_SPHINCS_ADDR_TYPE_FORSPK);

    fsmsw_sphincssha2_256fsimple_MessageToIndices(indices, m);

    for (i = 0; i < FSMSW_SPHINCSSHA2_256FSIMPLE_FORS_TREES; i++)
    {
        idx_offset = i * (uint32)((uint64)((uint64)1u << FSMSW_SPHINCSSHA2_256FSIMPLE_FORS_HEIGHT));

        FsmSw_SphincsSha2_set_tree_height(fors_tree_addr, 0);
        FsmSw_SphincsSha2_set_tree_index(fors_tree_addr, indices[i] + idx_offset);
        FsmSw_SphincsSha2_set_type(fors_tree_addr, FSMSW_SPHINCS_ADDR_TYPE_FORSPRF);

        /* Include the secret key part that produces the selected leaf node. */
        fsmsw_sphincssha2_256fsimple_ForsGenSk(sig_temp, ctx, fors_tree_addr);
        FsmSw_SphincsSha2_set_type(fors_tree_addr, FSMSW_SPHINCS_ADDR_TYPE_FORSTREE);
        sig_temp = &sig_temp[FSMSW_SPHINCSSHA2_256FSIMPLE_N];

        /* Compute the authentication path for this leaf node. */
        FsmSw_SphincsSha2_256fSimple_treehashx1(&roots[i * FSMSW_SPHINCSSHA2_256FSIMPLE_N], sig_temp, ctx,
                                                 indices[i], idx_offset, FSMSW_SPHINCSSHA2_256FSIMPLE_FORS_HEIGHT,
                                                 fsmsw_sphincssha2_256fsimple_ForsGenLeafx1, 
...
/***********************************************************************************************************************
* Name:        FsmSw_SphincsSha2_256fSimple_fors_pk_from_sig
*
                                                   ...
                                                   const sphincs_sha2_256f_ctx *ctx, const uint32 fors_addr[8])
{
    uint32 indices[FSMSW_SPHINCSSHA2_256FSIMPLE_FORS_TREES];
    uint8 roots[FSMSW_SPHINCSSHA2_256FSIMPLE_FORS_TREES * FSMSW_SPHINCSSHA2_256FSIMPLE_N];
    uint8 leaf[FSMSW_SPHINCSSHA2_256FSIMPLE_N];
    uint32 fors_tree_addr[8] = {0};
    uint32 fors_pk_addr[8] = {0};
    uint32 idx_offset;
    uint32 i;
    ...
    for (i = 0; i < FSMSW_SPHINCSSHA2_256FSIMPLE_FORS_TREES; i++)
    {
        ...
        fsmsw_sphincssha2_256fsimple_ForsSkToLeaf(leaf, sig_temp, ctx, fors_tree_addr);
        sig_temp = &sig_temp[FSMSW_SPHINCSSHA2_256FSIMPLE_N];

        /* Derive the corresponding root node of this tree. */
        FsmSw_SphincsSha2_256fSimple_compute_root(&roots[i * FSMSW_SPHINCSSHA2_256FSIMPLE_N], leaf, indices[i],
                                                  idx_offset, sig_temp, FSMSW_SPHINCSSHA2_256FSIMPLE_FORS_HEIGHT, ctx,
                                                  fors_tree_addr);
        sig_temp = &sig_temp[FSMSW_SPHINCSSHA2_256FSIMPLE_N * FSMSW_SPHINCSSHA2_256FSIMPLE_FORS_HEIGHT];
    }
