...
/**********************************************************************************************************************/
/***********************************************************************************************************************
* Name:        FsmSw_SphincsSha2_256fSimple_compute_root
*
...
*
***********************************************************************************************************************/
void FsmSw_SphincsSha2_256fSimple_compute_root(uint8 *root, const uint8 *leaf, uint32 leaf_idx, uint32 idx_offset,
                                               const uint8 *auth_path, uint32 tree_height,
                                               const sphincs_sha2_256f_ctx *ctx, uint32 addr[8])
{
    uint32 i;
    uint8 buffer[2u * FSMSW_SPHINCSSHA2_256FSIMPLE_N];
    ...
    if(0u < (leaf_idx & 1u))
    {
        ...
        FsmSw_CommonLib_memcpy(buffer, auth_path, FSMSW_SPHINCSSHA2_256FSIMPLE_N);
    }
    else
    {
        FsmSw_CommonLib_memcpy(buffer, leaf, FSMSW_SPHINCSSHA2_256FSIMPLE_N);
        FsmSw_CommonLib_memcpy(&buffer[FSMSW_SPHINCSSHA2_256FSIMPLE_N], auth_path, FSMSW_SPHINCSSHA2_256FSIMPLE_N);
    }
    auth_path = &auth_path[FSMSW_SPHINCSSHA2_256FSIMPLE_N];
...
/***********************************************************************************************************************
* Name:        FsmSw_SphincsSha2_256fSimple_treehash
*
...
void FsmSw_SphincsSha2_256fSimple_treehash(uint8 *root, uint8 *auth_path, const sphincs_sha2_256f_ctx *ctx,
                                           ...
                                           uint32 tree_addr[8])
{
    uint8  stack[(FSMSW_SPHINCSSHA2_256FSIMPLE_TREEHASH_BUF_LEN + 1u)*FSMSW_SPHINCSSHA2_256FSIMPLE_N];
    uint32 heights[FSMSW_SPHINCSSHA2_256FSIMPLE_TREEHASH_BUF_LEN + 1u];
    uint32 offset = 0;
    uint32 idx;
    uint32 tree_idx;
    ...
    for (idx = 0; idx < (uint32)((uint32)1u << tree_height); idx++)
    {
        ...
        while (offset >= 2u && heights[offset - 1u] == heights[offset - 2u])
        {
            ...
            FsmSw_SphincsSha2_set_tree_index(tree_addr, tree_idx + (idx_offset >> (heights[offset - 1u] + 1u)));
            /* Hash the top-most nodes from the stack together. */
            FsmSw_SphincsSha2_256fSimple_thash(&stack[(offset - 2u)*FSMSW_SPHINCSSHA2_256FSIMPLE_N],
                                               &stack[(offset - 2u)*FSMSW_SPHINCSSHA2_256FSIMPLE_N], 2u, ctx,
                                               tree_addr);
            offset--;
            ...
            if (((leaf_idx >> heights[offset - 1u]) ^ 0x1u) == tree_idx)
            {
                FsmSw_CommonLib_memcpy(&auth_path[heights[offset - 1u]*FSMSW_SPHINCSSHA2_256FSIMPLE_N],
                                       &stack[(offset - 1u)*FSMSW_SPHINCSSHA2_256FSIMPLE_N],
                                       FSMSW_SPHINCSSHA2_256FSIMPLE_N);
            }
        }
    }
