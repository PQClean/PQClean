...
/**********************************************************************************************************************/
static void fors_gen_sk(uint8 *sk, const sphincs_shake_128f_ctx *ctx, uint32 fors_leaf_addr[8]);
static void fors_sk_to_leaf(uint8 *leaf, const uint8 *sk, const sphincs_shake_128f_ctx *ctx, uint32 fors_leaf_addr[8]);
static void fors_gen_leafx1(uint8 *leaf, const sphincs_shake_128f_ctx *ctx, uint32 addr_idx, void *info);
                                            ...
                                            const uint32 fors_addr[8])
{
    ...
    for (i = 0; i < FSMSW_SPHINCSSHAKE_128FSIMPLE_FORS_TREES; i++)
    {
        ...
        FsmSw_SphincsShake_128fSimple_treehashx1(&roots[i * FSMSW_SPHINCSSHAKE_128FSIMPLE_N], sig, ctx, indices[i],
                                                 idx_offset, FSMSW_SPHINCSSHAKE_128FSIMPLE_FORS_HEIGHT, fors_gen_leafx1,
                                                 fors_tree_addr, &fors_info);

        sig = &sig[FSMSW_SPHINCSSHAKE_128FSIMPLE_N * FSMSW_SPHINCSSHAKE_128FSIMPLE_FORS_HEIGHT];
    }
                                                    ...
                                                    const sphincs_shake_128f_ctx *ctx, const uint32 fors_addr[8])
{
    ...
    for (i = 0; i < FSMSW_SPHINCSSHAKE_128FSIMPLE_FORS_TREES; i++)
    {
        ...
        FsmSw_SphincsShake_128fSimple_compute_root(&roots[i * FSMSW_SPHINCSSHAKE_128FSIMPLE_N], leaf, indices[i],
                                                   idx_offset, sig, FSMSW_SPHINCSSHAKE_128FSIMPLE_FORS_HEIGHT, ctx,
                                                   fors_tree_addr);
        sig = &sig[FSMSW_SPHINCSSHAKE_128FSIMPLE_N * FSMSW_SPHINCSSHAKE_128FSIMPLE_FORS_HEIGHT];
    }
