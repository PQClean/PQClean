...
/**********************************************************************************************************************/
static void gen_chain(uint8 *out, const uint8 *in, uint32 start, uint32 steps, const sphincs_shake_128s_ctx *ctx,
                      uint32 addr[8]);
...
static void wots_checksum(uint32 *csum_base_w, const uint32 *msg_base_w)
{
    ...
    base_w(csum_base_w, (sint32)FSMSW_SPHINCSSHAKE_128SSIMPLE_WOTS_LEN2, csum_bytes);
}
...
{
    base_w(lengths, (sint32)FSMSW_SPHINCSSHAKE_128SSIMPLE_WOTS_LEN1, msg);
    wots_checksum(&lengths[FSMSW_SPHINCSSHAKE_128SSIMPLE_WOTS_LEN1], lengths);
}
...
void FsmSw_SphincsShake_128sSimple_wots_pk_from_sig(uint8 *pk, const uint8 *sig, const uint8 *msg,
    ...
    for (i = 0; i < FSMSW_SPHINCSSHAKE_128SSIMPLE_WOTS_LEN; i++)
    {
        ...
        gen_chain(&pk[i * FSMSW_SPHINCSSHAKE_128SSIMPLE_N], &sig[i * FSMSW_SPHINCSSHAKE_128SSIMPLE_N],
                  lengths[i], FSMSW_SPHINCSSHAKE_128SSIMPLE_WOTS_W - 1u - lengths[i], ctx, addr);
    }
}
