                                                     ...
                                                     const uint8 *m, uint32 mlen, const sphincs_sha2_192f_ctx *ctx)
{
    ...
    if (FSMSW_SPHINCSSHA2_192FSIMPLE_N + mlen < SPX_SHAX_BLOCK_BYTES)
    {
        FsmSw_CommonLib_memcpy(&buf[FSMSW_SPHINCSSHA2_192FSIMPLE_N], m, mlen);
        shaX_inc_finalize(&buf[SPX_SHAX_BLOCK_BYTES], &state,
                          buf, mlen + FSMSW_SPHINCSSHA2_192FSIMPLE_N);
    }
    /* Otherwise first fill a block, so that finalize only uses the message */
    else
    {
        FsmSw_CommonLib_memcpy(&buf[FSMSW_SPHINCSSHA2_192FSIMPLE_N], m,
                               SPX_SHAX_BLOCK_BYTES - FSMSW_SPHINCSSHA2_192FSIMPLE_N);
        shaX_inc_blocks(&state, buf, 1);

        m = &m[SPX_SHAX_BLOCK_BYTES - FSMSW_SPHINCSSHA2_192FSIMPLE_N];
        mlen -= SPX_SHAX_BLOCK_BYTES - FSMSW_SPHINCSSHA2_192FSIMPLE_N;
                                               ...
                                               const sphincs_sha2_192f_ctx *ctx)
{
    ...
    if (FSMSW_SPHINCSSHA2_192FSIMPLE_N + FSMSW_SPHINCSSHA2_192FSIMPLE_PK_BYTES + mlen <
        SPX_INBLOCKS * SPX_SHAX_BLOCK_BYTES)
    {
        FsmSw_CommonLib_memcpy(&inbuf[FSMSW_SPHINCSSHA2_192FSIMPLE_N + FSMSW_SPHINCSSHA2_192FSIMPLE_PK_BYTES], m, mlen);
        shaX_inc_finalize(&seed[2u * FSMSW_SPHINCSSHA2_192FSIMPLE_N], &state, inbuf, FSMSW_SPHINCSSHA2_192FSIMPLE_N +
                          FSMSW_SPHINCSSHA2_192FSIMPLE_PK_BYTES + mlen);
    }
    /* Otherwise first fill a block, so that finalize only uses the message */
    else
    {
        FsmSw_CommonLib_memcpy(&inbuf[FSMSW_SPHINCSSHA2_192FSIMPLE_N + FSMSW_SPHINCSSHA2_192FSIMPLE_PK_BYTES], m,
                               SPX_INBLOCKS * SPX_SHAX_BLOCK_BYTES - FSMSW_SPHINCSSHA2_192FSIMPLE_N -
                               FSMSW_SPHINCSSHA2_192FSIMPLE_PK_BYTES);
        shaX_inc_blocks(&state, inbuf, SPX_INBLOCKS);

        m = &m[SPX_INBLOCKS * SPX_SHAX_BLOCK_BYTES - FSMSW_SPHINCSSHA2_192FSIMPLE_N -
               FSMSW_SPHINCSSHA2_192FSIMPLE_PK_BYTES];
        mlen -= SPX_INBLOCKS * SPX_SHAX_BLOCK_BYTES - FSMSW_SPHINCSSHA2_192FSIMPLE_N -
                FSMSW_SPHINCSSHA2_192FSIMPLE_PK_BYTES;
        shaX_inc_finalize(&seed[2u * FSMSW_SPHINCSSHA2_192FSIMPLE_N], &state, m, (uint32)mlen);
    }
