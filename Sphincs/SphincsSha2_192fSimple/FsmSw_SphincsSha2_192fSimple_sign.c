                                                       ...
                                                       const uint8 *sk)
{
    ...
    FsmSw_SphincsSha2_192fSimple_gen_message_random(sig, sk_prf, optrand, m, mlen, &ctx);

    /* Derive the message digest and leaf index from R, PK and M. */
    FsmSw_SphincsSha2_192fSimple_hash_message(mhash, &tree, &idx_leaf, sig, pk, m, mlen, &ctx);
    ...
    FsmSw_SphincsSha2_192fSimple_fors_sign(sig, root, mhash, &ctx, wots_addr);
    sig = &sig[FSMSW_SPHINCSSHA2_192FSIMPLE_FORS_BYTES];
    ...
    for (i = 0; i < FSMSW_SPHINCSSHA2_192FSIMPLE_D; i++)
    {
        ...
        FsmSw_SphincsSha2_192fSimple_merkle_sign(sig, root, &ctx, wots_addr, tree_addr, idx_leaf);
        sig = &sig[FSMSW_SPHINCSSHA2_192FSIMPLE_WOTS_BYTES + FSMSW_SPHINCSSHA2_192FSIMPLE_TREE_HEIGHT *
                                                    ...
                                                    const uint8 *pk)
{
    ...
    FsmSw_SphincsSha2_192fSimple_fors_pk_from_sig(root, sig, mhash, &ctx, wots_addr);
    sig = &sig[FSMSW_SPHINCSSHA2_192FSIMPLE_FORS_BYTES];
    ...
    for (i = 0; i < FSMSW_SPHINCSSHA2_192FSIMPLE_D; i++)
    {
        ...
        FsmSw_SphincsSha2_192fSimple_wots_pk_from_sig(wots_pk, sig, root, &ctx, wots_addr);
        sig = &sig[FSMSW_SPHINCSSHA2_192FSIMPLE_WOTS_BYTES];

        /* Compute the leaf node using the WOTS public key. */
        FsmSw_SphincsSha2_192fSimple_thash(leaf, wots_pk, FSMSW_SPHINCSSHA2_192FSIMPLE_WOTS_LEN, &ctx, wots_pk_addr);

        /* Compute the root node of this subtree. */
        FsmSw_SphincsSha2_192fSimple_compute_root(root, leaf, idx_leaf, 0, sig,
                                                  FSMSW_SPHINCSSHA2_192FSIMPLE_TREE_HEIGHT, &ctx, tree_addr);
        sig = &sig[FSMSW_SPHINCSSHA2_192FSIMPLE_TREE_HEIGHT * FSMSW_SPHINCSSHA2_192FSIMPLE_N];
