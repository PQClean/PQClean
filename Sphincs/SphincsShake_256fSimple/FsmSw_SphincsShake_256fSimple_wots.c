...
/**********************************************************************************************************************/
static void gen_chain(uint8 *out, const uint8 *in, uint32 start, uint32 steps, const sphincs_shake_256f_ctx *ctx,
                      uint32 addr[8]);
...
static void wots_checksum(uint32 *csum_base_w, const uint32 *msg_base_w)
{
    ...
    csum = csum << ((8u - ((FSMSW_SPHINCSSHAKE_256FSIMPLE_WOTS_LEN2 * FSMSW_SPHINCSSHAKE_256FSIMPLE_WOTS_LOGW) % 8u))
                     % 8u);
    FsmSw_Sphincs_ull_to_bytes(csum_bytes, sizeof(csum_bytes), csum);
    base_w(csum_base_w, (sint32)FSMSW_SPHINCSSHAKE_256FSIMPLE_WOTS_LEN2, csum_bytes);
}
...
/**********************************************************************************************************************/
/***********************************************************************************************************************
* Name:        FsmSw_SphincsShake_256fSimple_chain_lengths
*
...
{
    base_w(lengths, (sint32)FSMSW_SPHINCSSHAKE_256FSIMPLE_WOTS_LEN1, msg);
    wots_checksum(&lengths[FSMSW_SPHINCSSHAKE_256FSIMPLE_WOTS_LEN1], lengths);
}

/***********************************************************************************************************************
* Name:        FsmSw_SphincsShake_256fSimple_wots_pk_from_sig
*
...
{
    uint32 lengths[FSMSW_SPHINCSSHAKE_256FSIMPLE_WOTS_LEN];
    uint32 i;

    FsmSw_SphincsShake_256fSimple_chain_lengths(lengths, msg);

    for (i = 0; i < FSMSW_SPHINCSSHAKE_256FSIMPLE_WOTS_LEN; i++)
    {
        FsmSw_SphincsShake_set_chain_addr(addr, i);
        gen_chain(&pk[i * FSMSW_SPHINCSSHAKE_256FSIMPLE_N], &sig[i * FSMSW_SPHINCSSHAKE_256FSIMPLE_N],
                  lengths[i], FSMSW_SPHINCSSHAKE_256FSIMPLE_WOTS_W - 1u - lengths[i], ctx, addr);
    }
}
