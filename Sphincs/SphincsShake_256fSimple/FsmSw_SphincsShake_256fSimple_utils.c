...
/**********************************************************************************************************************/
/***********************************************************************************************************************
* Name:        FsmSw_SphincsShake_256fSimple_compute_root
*
...
void FsmSw_SphincsShake_256fSimple_compute_root(uint8 *root, const uint8 *leaf, uint32 leaf_idx, uint32 idx_offset,
...
/***********************************************************************************************************************
* Name:        FsmSw_SphincsShake_256fSimple_treehash
*
...
void FsmSw_SphincsShake_256fSimple_treehash(uint8 *root, uint8 *auth_path, const sphincs_shake_256f_ctx *ctx,
                                            ...
                                            uint32 tree_addr[8])
{
    uint8  stack[(FSMSW_SPHINCSSHAKE_256FSIMPLE_TREEHASH_BUF_LEN + 1u)*FSMSW_SPHINCSSHAKE_256FSIMPLE_N];
    uint32 heights[FSMSW_SPHINCSSHAKE_256FSIMPLE_TREEHASH_BUF_LEN + 1u];
    uint32 offset = 0;
    uint32 idx;
    uint32 tree_idx;
    ...
    for (idx = 0; idx < (uint32)((uint32)1u << tree_height); idx++)
    {
        ...
        while ((offset >= 2u) && (heights[offset - 1u] == heights[offset - 2u]))
        {
            ...
            FsmSw_SphincsShake_256fSimple_thash(&stack[(offset - 2u)*FSMSW_SPHINCSSHAKE_256FSIMPLE_N],
                                                &stack[(offset - 2u)*FSMSW_SPHINCSSHAKE_256FSIMPLE_N],
                                                2u, ctx, tree_addr);
            offset--;
