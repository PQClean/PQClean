...
/**********************************************************************************************************************/
static sint8 FsmSw_SphincsShake_192fSimple_crypto_sign_seed_keypair(uint8 *pk, uint8 *sk, const uint8 *seed);
/**********************************************************************************************************************/
...
*
***********************************************************************************************************************/
sint8 FsmSw_SphincsShake_192fSimple_crypto_sign_keypair(uint8 *pk, uint8 *sk)
{
                                                        ...
                                                        const uint8 *sk)
{
    ...
    FsmSw_SphincsShake_192fSimple_gen_message_random(sig_temp, sk_prf, optrand, m, mlen, &ctx);

    /* Derive the message digest and leaf index from R, PK and M. */
    FsmSw_SphincsShake_192fSimple_hash_message(mhash, &tree, &idx_leaf, sig_temp, pk, m, mlen, &ctx);
    ...
    for (i = 0; i < FSMSW_SPHINCSSHAKE_192FSIMPLE_D; i++)
    {
        ...
        sig_temp = &sig_temp[FSMSW_SPHINCSSHAKE_192FSIMPLE_WOTS_BYTES + (FSMSW_SPHINCSSHAKE_192FSIMPLE_TREE_HEIGHT *
                   FSMSW_SPHINCSSHAKE_192FSIMPLE_N)];
                                                     ...
                                                     const uint8 *pk)
{
    ...
    FsmSw_SphincsShake_192fSimple_fors_pk_from_sig(root, sig_temp, mhash, &ctx, wots_addr);
    sig_temp = &sig_temp[FSMSW_SPHINCSSHAKE_192FSIMPLE_FORS_BYTES];
    ...
    for (i = 0; i < FSMSW_SPHINCSSHAKE_192FSIMPLE_D; i++)
    {
        ...
        FsmSw_SphincsShake_192fSimple_wots_pk_from_sig(wots_pk, sig_temp, root, &ctx, wots_addr);
        ...
        FsmSw_SphincsShake_192fSimple_compute_root(root, leaf, idx_leaf, 0, sig_temp,
