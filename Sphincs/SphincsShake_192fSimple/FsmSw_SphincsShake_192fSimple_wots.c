...
/**********************************************************************************************************************/
static void gen_chain(uint8 *out, const uint8 *in, uint32 start, uint32 steps, const sphincs_shake_192f_ctx *ctx,
                      uint32 addr[8]);
...
static void wots_checksum(uint32 *csum_base_w, const uint32 *msg_base_w)
{
    ...
    base_w(csum_base_w, (sint32)FSMSW_SPHINCSSHAKE_192FSIMPLE_WOTS_LEN2, csum_bytes);
}
...
{
    base_w(lengths, (sint32)FSMSW_SPHINCSSHAKE_192FSIMPLE_WOTS_LEN1, msg);
    wots_checksum(&lengths[FSMSW_SPHINCSSHAKE_192FSIMPLE_WOTS_LEN1], lengths);
}
                                                    ...
                                                    const sphincs_shake_192f_ctx *ctx, uint32 addr[8])
{
    ...
    for (i = 0; i < FSMSW_SPHINCSSHAKE_192FSIMPLE_WOTS_LEN; i++)
    {
        ...
        gen_chain(&pk[i * FSMSW_SPHINCSSHAKE_192FSIMPLE_N], &sig[i * FSMSW_SPHINCSSHAKE_192FSIMPLE_N],
                  lengths[i], FSMSW_SPHINCSSHAKE_192FSIMPLE_WOTS_W - 1u - lengths[i], ctx, addr);
    }
}
