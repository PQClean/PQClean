                                                ...
                                                const sphincs_shake_192f_ctx *ctx, uint32 addr[8])
{
    ...
    if (0u < (leaf_idx & 1u))
    {
        ...
        FsmSw_CommonLib_memcpy(buffer, auth_path, FSMSW_SPHINCSSHAKE_192FSIMPLE_N);
    }
    else
    {
        FsmSw_CommonLib_memcpy(buffer, leaf, FSMSW_SPHINCSSHAKE_192FSIMPLE_N);
        FsmSw_CommonLib_memcpy(&buffer[FSMSW_SPHINCSSHAKE_192FSIMPLE_N], auth_path, FSMSW_SPHINCSSHAKE_192FSIMPLE_N);
    }
    auth_path = &auth_path[FSMSW_SPHINCSSHAKE_192FSIMPLE_N];
    ...
    for (i = 0; i < tree_height - 1u; i++)
    {
        ...
        else
        {
            ...
            FsmSw_CommonLib_memcpy(&buffer[FSMSW_SPHINCSSHAKE_192FSIMPLE_N], auth_path,
