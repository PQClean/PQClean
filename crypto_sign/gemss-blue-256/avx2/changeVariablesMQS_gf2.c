 ...
 */
int PQCLEAN_GEMSSBLUE256_AVX2_changeVariablesMQS_simd_gf2(mqsnv_gf2n MQS, cst_GLnv_gf2 S) {
 ...
 * @return   0 if the result is correct, ERROR_ALLOC for error from
 * malloc/calloc functions.
 * @remark  This function should be faster than PQCLEAN_GEMSSBLUE256_AVX2_changeVariablesMQS_simd_gf2
 * when SIMD is not used.
 * @remark  Constant-time implementation.
 */
int PQCLEAN_GEMSSBLUE256_AVX2_changeVariablesMQS64_gf2(mqsnv_gf2n MQS, cst_GLnv_gf2 S) {
    /* Tmp matrix (n+v)*(n+v) of quadratic terms to compute S*Q */
    UINT MQS2[HFEnv * HFEnv * NB_WORD_GFqn] = {0};
    UINT *MQS2_cp;
    UINT bit_kr, mask;
