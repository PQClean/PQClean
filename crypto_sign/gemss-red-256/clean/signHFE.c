...
void PQCLEAN_GEMSSRED256_CLEAN_precSignHFESeed(secret_key_HFE *sk_HFE, const UINT *sk) {
    ...
    PQCLEAN_GEMSSRED256_CLEAN_cleanMonicHFEv_gf2nx(sk_HFE->F_HFEv);
    ...
    PQCLEAN_GEMSSRED256_CLEAN_cleanLowerMatrixnv(L);
    PQCLEAN_GEMSSRED256_CLEAN_cleanLowerMatrixnv(U);

    /* Generate S^(-1) = L*U */
    PQCLEAN_GEMSSRED256_CLEAN_mulMatricesLU_gf2_nv(sk_HFE->S, L, U);
    ...
    PQCLEAN_GEMSSRED256_CLEAN_cleanLowerMatrixn(L);
    PQCLEAN_GEMSSRED256_CLEAN_cleanLowerMatrixn(U);

    /* Generate T^(-1) = L*U */
    PQCLEAN_GEMSSRED256_CLEAN_mulMatricesLU_gf2_n(sk_HFE->T, L, U);
...
int PQCLEAN_GEMSSRED256_CLEAN_precSignHFE(secret_key_HFE *sk_HFE, const UINT **linear_coefs,
    ...
    PQCLEAN_GEMSSRED256_CLEAN_precSignHFESeed(sk_HFE, sk);

    PQCLEAN_GEMSSRED256_CLEAN_initListDifferences_gf2nx(sk_HFE->F_struct.L);
...
int PQCLEAN_GEMSSRED256_CLEAN_signHFE_FeistelPatarin(unsigned char *sm8,
    ...
    nb_root = PQCLEAN_GEMSSRED256_CLEAN_precSignHFE(&sk_HFE, linear_coefs, sk);
    if (nb_root) {
        /* Error from malloc */
        return nb_root;
    }
    ...
    for (k = 1; k <= NB_ITE; ++k) {
        ...
        do {
            ...
            nb_root = PQCLEAN_GEMSSRED256_CLEAN_chooseRootHFE_gf2nx(DR, sk_HFE.F_struct, U);
            if (!nb_root) {
