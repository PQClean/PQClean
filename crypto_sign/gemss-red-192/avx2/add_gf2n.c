 ...
 */
void PQCLEAN_GEMSSRED192_AVX2_add_gf2n_ref(gf2n c, cst_gf2n a, cst_gf2n b) {
    unsigned int i;
 ...
 * @remark  Constant-time implementation.
 */
void PQCLEAN_GEMSSRED192_AVX2_add2_gf2n_ref(gf2n c, cst_gf2n a) {
    unsigned int i;
    for (i = 0; i < NB_WORD_GFqn; ++i) {
        c[i] ^= a[i];
    }
 ...
 * @remark  Constant-time implementation.
 */
void PQCLEAN_GEMSSRED192_AVX2_add_product_gf2n_ref(gf2x c, cst_gf2x a, cst_gf2x b) {
    unsigned int i;
 ...
 * @remark  Constant-time implementation.
 */
void PQCLEAN_GEMSSRED192_AVX2_add2_product_gf2n_ref(gf2x c, cst_gf2x a) {
    unsigned int i;
    for (i = 0; i < NB_WORD_MMUL; ++i) {
        c[i] ^= a[i];
    }
