...
void PQCLEAN_DILITHIUM4_CLEAN_challenge(poly *c,
    ...
    PQCLEAN_DILITHIUM4_CLEAN_shake256_absorb(&state, inbuf, sizeof(inbuf));
    PQCLEAN_DILITHIUM4_CLEAN_shake256_squeezeblocks(outbuf, 1, &state);
...
int PQCLEAN_DILITHIUM4_CLEAN_crypto_sign_keypair(uint8_t *pk, uint8_t *sk) {
    ...
    PQCLEAN_DILITHIUM4_CLEAN_crh(tr, pk, CRYPTO_PUBLICKEYBYTES);
...
int PQCLEAN_DILITHIUM4_CLEAN_crypto_sign_signature(
    ...
    PQCLEAN_DILITHIUM4_CLEAN_crh(rhoprime, key, SEEDBYTES + CRHBYTES);
