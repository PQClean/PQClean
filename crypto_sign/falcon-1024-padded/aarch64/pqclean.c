...
/* see api.h */
int
PQCLEAN_FALCON1024PADDED_AARCH64_crypto_sign_keypair(
    ...
    PQCLEAN_FALCON1024PADDED_AARCH64_keygen(&rng, f, g, F, NULL, h, FALCON_LOGN, tmp.b);
    ...
    u = 1;
    v = PQCLEAN_FALCON1024PADDED_AARCH64_trim_i8_encode(
            sk + u, PQCLEAN_FALCON1024PADDED_AARCH64_CRYPTO_SECRETKEYBYTES - u,
            f, PQCLEAN_FALCON1024PADDED_AARCH64_max_fg_bits[FALCON_LOGN]);
    if (v == 0) {
        return -1;
    }
    u += v;
    v = PQCLEAN_FALCON1024PADDED_AARCH64_trim_i8_encode(
            sk + u, PQCLEAN_FALCON1024PADDED_AARCH64_CRYPTO_SECRETKEYBYTES - u,
            g, PQCLEAN_FALCON1024PADDED_AARCH64_max_fg_bits[FALCON_LOGN]);
    if (v == 0) {
        return -1;
    }
    u += v;
    ...
    u += v;
    if (u != PQCLEAN_FALCON1024PADDED_AARCH64_CRYPTO_SECRETKEYBYTES) {
        return -1;
    }
    ...
    v = PQCLEAN_FALCON1024PADDED_AARCH64_modq_encode(
            pk + 1, PQCLEAN_FALCON1024PADDED_AARCH64_CRYPTO_PUBLICKEYBYTES - 1,
            h, FALCON_LOGN);
    if (v != PQCLEAN_FALCON1024PADDED_AARCH64_CRYPTO_PUBLICKEYBYTES - 1) {
        return -1;
    }
...
do_sign(uint8_t *nonce, uint8_t *sigbuf, size_t sigbuflen,
    ...
    u = 1;
    v = PQCLEAN_FALCON1024PADDED_AARCH64_trim_i8_decode(
            f, PQCLEAN_FALCON1024PADDED_AARCH64_max_fg_bits[FALCON_LOGN],
            sk + u, PQCLEAN_FALCON1024PADDED_AARCH64_CRYPTO_SECRETKEYBYTES - u);
    if (v == 0) {
        return -1;
    ...
    if (!PQCLEAN_FALCON1024PADDED_AARCH64_complete_private(G, f, g, F, tmp.b)) {
        return -1;
    }
    ...
    PQCLEAN_FALCON1024PADDED_AARCH64_hash_to_point_ct(&sc, r.hm, FALCON_LOGN, tmp.b);
     ...
     */
    for (;;) {
        PQCLEAN_FALCON1024PADDED_AARCH64_sign_dyn(r.sig, &sc, f, g, F, G, r.hm, tmp.b);
        v = PQCLEAN_FALCON1024PADDED_AARCH64_comp_encode(sigbuf, sigbuflen, r.sig);
        if (v != 0) {
            inner_shake256_ctx_release(&sc);
...
do_verify(
    ...
    if (PQCLEAN_FALCON1024PADDED_AARCH64_modq_decode( (uint16_t *) h,
            pk + 1, PQCLEAN_FALCON1024PADDED_AARCH64_CRYPTO_PUBLICKEYBYTES - 1, FALCON_LOGN)
    ...
    inner_shake256_inject(&sc, m, mlen);
    inner_shake256_flip(&sc);
    PQCLEAN_FALCON1024PADDED_AARCH64_hash_to_point_ct(&sc, (uint16_t *) hm, FALCON_LOGN, tmp.b);
    inner_shake256_ctx_release(&sc);

    /*
     * Verify signature.
     */
    if (!PQCLEAN_FALCON1024PADDED_AARCH64_verify_raw(hm, sig, h, (int16_t *) tmp.b)) {
        return -1;
    }
    return 0;
}
...
/* see api.h */
int
PQCLEAN_FALCON1024PADDED_AARCH64_crypto_sign_signature(
    uint8_t *sig, size_t *siglen,
    const uint8_t *m, size_t mlen, const uint8_t *sk) {
    size_t vlen;

    vlen = PQCLEAN_FALCON1024PADDED_AARCH64_CRYPTO_BYTES - NONCELEN - 1;
...
/* see api.h */
int
PQCLEAN_FALCON1024PADDED_AARCH64_crypto_sign_verify(
...
/* see api.h */
int
PQCLEAN_FALCON1024PADDED_AARCH64_crypto_sign(
     ...
     */
    memmove(sm + PQCLEAN_FALCON1024PADDED_AARCH64_CRYPTO_BYTES, m, mlen);
...
/* see api.h */
int
PQCLEAN_FALCON1024PADDED_AARCH64_crypto_sign_open(
    uint8_t *m, size_t *mlen,
    const uint8_t *sm, size_t smlen, const uint8_t *pk) {
    const uint8_t *sigbuf;
    size_t pmlen, sigbuflen;

    if (smlen < PQCLEAN_FALCON1024PADDED_AARCH64_CRYPTO_BYTES) {
        return -1;
    }
    sigbuflen = PQCLEAN_FALCON1024PADDED_AARCH64_CRYPTO_BYTES - NONCELEN - 1;
    ...
    if (do_verify(sm + 1, sigbuf, sigbuflen,
                  sm + PQCLEAN_FALCON1024PADDED_AARCH64_CRYPTO_BYTES, pmlen, pk) < 0) {
        return -1;
    }
     ...
     * overlaps.
     */
    memmove(m, sm + PQCLEAN_FALCON1024PADDED_AARCH64_CRYPTO_BYTES, pmlen);
    *mlen = pmlen;
    return 0;
}
