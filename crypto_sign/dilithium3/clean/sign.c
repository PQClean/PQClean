...
void PQCLEAN_DILITHIUM3_CLEAN_challenge(poly *c,
    ...
    PQCLEAN_DILITHIUM3_CLEAN_shake256_absorb(&state, inbuf, sizeof(inbuf));
    PQCLEAN_DILITHIUM3_CLEAN_shake256_squeezeblocks(outbuf, 1, &state);
...
int PQCLEAN_DILITHIUM3_CLEAN_crypto_sign_keypair(uint8_t *pk, uint8_t *sk) {
    ...
    PQCLEAN_DILITHIUM3_CLEAN_crh(tr, pk, CRYPTO_PUBLICKEYBYTES);
...
int PQCLEAN_DILITHIUM3_CLEAN_crypto_sign_signature(
    ...
    uint64_t s[26];
    shake256_inc_init(s);
    shake256_inc_absorb(s, tr, CRHBYTES);
    shake256_inc_absorb(s, m, mlen);
    shake256_inc_finalize(s);
    shake256_inc_squeeze(mu, CRHBYTES, s);
    ...
    PQCLEAN_DILITHIUM3_CLEAN_crh(rhoprime, key, SEEDBYTES + CRHBYTES);
...
int PQCLEAN_DILITHIUM3_CLEAN_crypto_sign_verify(
    ...
    uint64_t s[26];
    shake256_inc_init(s);
    shake256_inc_absorb(s, pk, CRYPTO_PUBLICKEYBYTES);
    shake256_inc_finalize(s);
    shake256_inc_squeeze(mu, CRHBYTES, s);

    shake256_inc_init(s);
    shake256_inc_absorb(s, mu, CRHBYTES);
    shake256_inc_absorb(s, m, mlen);
    shake256_inc_finalize(s);
    shake256_inc_squeeze(mu, CRHBYTES, s);
