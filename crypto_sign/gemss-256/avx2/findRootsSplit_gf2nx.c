...
static void traceMap_gf2nx(gf2nx poly_trace, gf2nx poly_frob,
    ...
    if (i < HFEn) {
        ...
        PQCLEAN_GEMSS256_AVX2_div_r_monic_cst_gf2nx(poly_frob, 1U << i, f, deg);
        /* poly_trace += ((rX)^(2^i)) mod f */
        ...
        for (; i < HFEn; ++i) {
            ...
            PQCLEAN_GEMSS256_AVX2_sqr_gf2nx(poly_frob, deg - 1);
            /* poly_frob = ((rX)^(2^i)) mod f */
            PQCLEAN_GEMSS256_AVX2_div_r_monic_cst_gf2nx(poly_frob, (deg - 1) << 1U, f, deg);
            /* poly_trace += ((rX)^(2^i)) mod f */
            add2_gf2nx(poly_trace, poly_frob, deg, j);
        }
    }
 ...
 */
void PQCLEAN_GEMSS256_AVX2_findRootsSplit_gf2nx(vec_gf2n roots, gf2nx f,
        unsigned int deg) {
    ...
    do {
        ...
        l = PQCLEAN_GEMSS256_AVX2_gcd_gf2nx(&b, f_cp, deg, poly_trace, d);
    ...
    PQCLEAN_GEMSS256_AVX2_div_q_monic_gf2nx(f, deg, f_cp, l);
    /* Necessarily, the polynomial f is null here */

    /* f_cp is monic */
    /* We can apply PQCLEAN_GEMSS256_AVX2_findRootsSplit_gf2nx recursively */
    PQCLEAN_GEMSS256_AVX2_findRootsSplit_gf2nx(roots, f_cp, l);

    /* f is monic and f_cp is monic so Q is monic */
    /* We can apply PQCLEAN_GEMSS256_AVX2_findRootsSplit_gf2nx recursively */
    PQCLEAN_GEMSS256_AVX2_findRootsSplit_gf2nx(roots + l * NB_WORD_GFqn, f + l * NB_WORD_GFqn, deg - l);
}
