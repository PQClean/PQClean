...
int PQCLEAN_GEMSS256_CLEAN_findRootsHFE_gf2nx(vec_gf2n *roots,
    ...
    /* Initialize to F */
    PQCLEAN_GEMSS256_CLEAN_convHFEpolynomialSparseToDense_gf2nx(poly2, F);
    /* Initialize to F-U */
    add2_gf2n(poly2, U);

    /* GCD(F-U, X^(2^n)-X mod (F-U)) */
    l = PQCLEAN_GEMSS256_CLEAN_gcd_gf2nx(&i, poly2, d2, poly, l);
    ...
    PQCLEAN_GEMSS256_CLEAN_convMonic_gf2nx(poly2, l);
    free(poly);
    ...
    *roots = (UINT *)malloc(l * NB_WORD_GFqn * sizeof(UINT));
    ...
    PQCLEAN_GEMSS256_CLEAN_findRootsSplit_gf2nx(*roots, poly2, l);
    free(poly2);
