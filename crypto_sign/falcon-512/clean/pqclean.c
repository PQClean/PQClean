...
PQCLEAN_FALCON512_CLEAN_crypto_sign_signature(
    ...
    sig[0] = 0x30 + 9;
    memset(sig + 1 + NONCELEN + vlen, 0, PQCLEAN_FALCON512_CLEAN_CRYPTO_BYTES - vlen - NONCELEN - 1);
...
PQCLEAN_FALCON512_CLEAN_crypto_sign(
    ...
    pm = sm + 2 + NONCELEN;
    sigbuf = pm + 1 + mlen;
    sigbuflen = PQCLEAN_FALCON512_CLEAN_MAX_CRYPTO_BYTES - NONCELEN - 1; // TODO: figure out proper constant length
