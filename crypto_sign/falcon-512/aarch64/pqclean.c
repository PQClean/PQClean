...
PQCLEAN_FALCON512_AARCH64_crypto_sign_keypair(
    ...
    PQCLEAN_FALCON512_AARCH64_keygen(&rng, f, g, F, NULL, h, 9, tmp.b);
    ...
    v = PQCLEAN_FALCON512_AARCH64_trim_i8_encode(
            ...
            f, PQCLEAN_FALCON512_AARCH64_max_fg_bits[9]);
    if (v == 0) {
        return -1;
    }
    u += v;
    v = PQCLEAN_FALCON512_AARCH64_trim_i8_encode(
            sk + u, PQCLEAN_FALCON512_AARCH64_CRYPTO_SECRETKEYBYTES - u,
            g, PQCLEAN_FALCON512_AARCH64_max_fg_bits[9]);
    if (v == 0) {
        return -1;
    }
    u += v;
...
do_sign(uint8_t *nonce, uint8_t *sigbuf, size_t *sigbuflen,
    ...
    PQCLEAN_FALCON512_AARCH64_hash_to_point_ct(&sc, r.hm, 9, tmp.b);
...
do_verify(
    ...
    PQCLEAN_FALCON512_AARCH64_to_ntt_monty(h);
    ...
    inner_shake256_inject(&sc, m, mlen);
    inner_shake256_flip(&sc);
    PQCLEAN_FALCON512_AARCH64_hash_to_point_ct(&sc, (uint16_t *) hm, 9, tmp.b);
    inner_shake256_ctx_release(&sc);
