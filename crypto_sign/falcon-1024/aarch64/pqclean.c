...
do_sign(uint8_t *nonce, uint8_t *sigbuf, size_t *sigbuflen,
     ...
     */
    PQCLEAN_FALCON1024_AARCH64_sign_dyn(r.sig, &sc, f, g, F, G, r.hm, FALCON_LOGN, tmp.b);
    v = PQCLEAN_FALCON1024_AARCH64_comp_encode(sigbuf, *sigbuflen, r.sig, FALCON_LOGN);
    if (v != 0) {
        inner_shake256_ctx_release(&sc);
        *sigbuflen = v;
        return 0;
