...
do_verify(
    ...
    if (v != sigbuflen) {
        if (sigbuflen == PQCLEAN_FALCON1024_AARCH64_CRYPTO_BYTES - NONCELEN - 1) {
            while (v < sigbuflen) {
                if (sigbuf[v++] != 0) {
                    return -1;
                }
...
PQCLEAN_FALCON1024_AARCH64_crypto_sign_signature(
    ...
    memset(sig + 1 + NONCELEN + vlen, 0, PQCLEAN_FALCON1024_AARCH64_CRYPTO_BYTES - vlen - NONCELEN - 1);
    *siglen = PQCLEAN_FALCON1024_AARCH64_CRYPTO_BYTES;
    return 0;
}
...
PQCLEAN_FALCON1024_AARCH64_crypto_sign_verify(
    ...
    const uint8_t *m, size_t mlen, const uint8_t *pk) {
    if (siglen != PQCLEAN_FALCON1024_AARCH64_CRYPTO_BYTES) { // TODO make permissive
        return -1;
    }
...
PQCLEAN_FALCON1024_AARCH64_crypto_sign(
    ...
    pm = sm + 2 + NONCELEN;
    sigbuf = pm + 1 + mlen;
    sigbuflen = PQCLEAN_FALCON1024_AARCH64_MAX_CRYPTO_BYTES - NONCELEN - 1; // TODO: figure out proper constant length
