...
int PQCLEAN_GEMSS128_CLEAN_crypto_sign_keypair(uint8_t *pk, uint8_t *sk) {
    return PQCLEAN_GEMSS128_CLEAN_sign_keypairHFE(pk, (UINT *)sk);
}
 ...
 * @return  Zero if the function runs correctly, non-zero else.
 */
int PQCLEAN_GEMSS128_CLEAN_crypto_sign(
    uint8_t *sm, size_t *smlen,
    const uint8_t *m, size_t mlen,
    const uint8_t *sk) {
    *smlen = mlen + PQCLEAN_GEMSS128_CLEAN_CRYPTO_BYTES;
    memmove(sm + PQCLEAN_GEMSS128_CLEAN_CRYPTO_BYTES, m, mlen);
 ...
 * @return  Zero if the function runs correctly, non-zero else.
 */
int PQCLEAN_GEMSS128_CLEAN_crypto_sign_open(
    uint8_t *m, size_t *mlen,
    const uint8_t *sm, size_t smlen,
    const uint8_t *pk) {
    int result;
    *mlen = smlen - PQCLEAN_GEMSS128_CLEAN_CRYPTO_BYTES;
    result = PQCLEAN_GEMSS128_CLEAN_sign_openHFE(sm + PQCLEAN_GEMSS128_CLEAN_CRYPTO_BYTES, (size_t)(*mlen), sm, pk);
    /* For compatibily with SUPERCOP, the memcpy is done only after sign_open */
    memmove(m, sm + PQCLEAN_GEMSS128_CLEAN_CRYPTO_BYTES, (size_t)(*mlen));
    return result;
}


int PQCLEAN_GEMSS128_CLEAN_crypto_sign_signature(uint8_t *sig, size_t *siglen, const uint8_t *m, size_t mlen, const uint8_t *sk) {
    int result;
    *siglen = PQCLEAN_GEMSS128_CLEAN_CRYPTO_BYTES;
    result = signHFE(sig, m, mlen, (UINT *)sk);
    return result;
}

int PQCLEAN_GEMSS128_CLEAN_crypto_sign_verify(const uint8_t *sig, size_t siglen, const uint8_t *m, size_t mlen, const uint8_t *pk) {
    int result;
    if (siglen == PQCLEAN_GEMSS128_CLEAN_CRYPTO_BYTES) {
        result = PQCLEAN_GEMSS128_CLEAN_sign_openHFE(m, mlen, sig, pk);
    } else {
        result = -1;
    }
    return result;
