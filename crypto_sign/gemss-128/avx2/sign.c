...
int PQCLEAN_GEMSS128_AVX2_crypto_sign_keypair(uint8_t *pk, uint8_t *sk) {
    return PQCLEAN_GEMSS128_AVX2_sign_keypairHFE(pk, (UINT *)sk);
}
 ...
 * @return  Zero if the function runs correctly, non-zero else.
 */
int PQCLEAN_GEMSS128_AVX2_crypto_sign(
 ...
 * @return  Zero if the function runs correctly, non-zero else.
 */
int PQCLEAN_GEMSS128_AVX2_crypto_sign_open(
...
int PQCLEAN_GEMSS128_AVX2_crypto_sign_signature(uint8_t *sig, size_t *siglen, const uint8_t *m, size_t mlen, const uint8_t *sk) {
    int result;
    *siglen = PQCLEAN_GEMSS128_AVX2_CRYPTO_BYTES;
...
int PQCLEAN_GEMSS128_AVX2_crypto_sign_verify(const uint8_t *sig, size_t siglen, const uint8_t *m, size_t mlen, const uint8_t *pk) {
    int result;
    if (siglen == PQCLEAN_GEMSS128_AVX2_CRYPTO_BYTES) {
        result = PQCLEAN_GEMSS128_AVX2_sign_openHFE(m, mlen, sig, pk);
    } else {
        result = -1;
    }
    return result;
