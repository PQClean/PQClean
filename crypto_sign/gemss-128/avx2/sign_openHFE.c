...
int PQCLEAN_GEMSS128_AVX2_sign_openHFE(const unsigned char *m, size_t len,
    ...
    for (i = 0; i < (HFEmr8 - 1); i++) {
        cst ^= PQCLEAN_GEMSS128_AVX2_convMQ_uncompressL_gf2(pk_tmp + 1 + i * NB_WORD_UNCOMP_EQ,
                pk + ACCESS_last_equations8 + i * NB_BYTES_EQUATION) << i;
    }

    /* The last equation in input is smaller because compressed */
    cst ^= PQCLEAN_GEMSS128_AVX2_convMQ_last_uncompressL_gf2(pk_tmp + 1 + i * NB_WORD_UNCOMP_EQ,
            pk + ACCESS_last_equations8 + i * NB_BYTES_EQUATION) << i;
    ...
    *pk_tmp = cst;
    ...
    ret = PQCLEAN_GEMSS128_AVX2_sign_openHFE_huncomp_pk(m, len, sm8, pk, (uint8_t *)pk_tmp,
