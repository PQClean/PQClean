...
static void PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_thash(
    ...
    if (inblocks == 1) {
        ...
        PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_haraka256(outbuf, buf_tmp, state_seeded);
        for (i = 0; i < inblocks * SPX_N; i++) {
            buf_tmp[SPX_ADDR_BYTES + i] = in[i] ^ outbuf[i];
        }
        PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_haraka512(outbuf, buf_tmp, state_seeded);
