...
void PQCLEAN_GEMSSRED128_CLEAN_precSignHFESeed(secret_key_HFE *sk_HFE, const UINT *sk) {
    Tnv_gf2 L, U;
    ...
    PQCLEAN_GEMSSRED128_CLEAN_cleanMonicHFEv_gf2nx(sk_HFE->F_HFEv);
    ...
    PQCLEAN_GEMSSRED128_CLEAN_cleanLowerMatrixnv(L);
    PQCLEAN_GEMSSRED128_CLEAN_cleanLowerMatrixnv(U);

    /* Generate S^(-1) = L*U */
    PQCLEAN_GEMSSRED128_CLEAN_mulMatricesLU_gf2_nv(sk_HFE->S, L, U);
    ...
    PQCLEAN_GEMSSRED128_CLEAN_cleanLowerMatrixn(L);
    PQCLEAN_GEMSSRED128_CLEAN_cleanLowerMatrixn(U);

    /* Generate T^(-1) = L*U */
    PQCLEAN_GEMSSRED128_CLEAN_mulMatricesLU_gf2_n(sk_HFE->T, L, U);
...
int PQCLEAN_GEMSSRED128_CLEAN_signHFE_FeistelPatarin(unsigned char *sm8,
    ...
    nb_root = PQCLEAN_GEMSSRED128_CLEAN_precSignHFE(&sk_HFE, linear_coefs, sk);
    if (nb_root) {
        /* Error from malloc */
        return nb_root;
    }
    ...
    for (k = 1; k <= NB_ITE; ++k) {
        ...
        do {
            ...
            nb_root = PQCLEAN_GEMSSRED128_CLEAN_chooseRootHFE_gf2nx(DR, sk_HFE.F_struct, U);
            if (!nb_root) {
