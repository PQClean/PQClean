...
/* see api.h */
int
PQCLEAN_FALCON512PADDED_CLEAN_crypto_sign_keypair(
    ...
    PQCLEAN_FALCON512PADDED_CLEAN_keygen(&rng, f, g, F, NULL, h, 9, tmp.b);
    ...
    sk[0] = 0x50 + 9;
    u = 1;
    v = PQCLEAN_FALCON512PADDED_CLEAN_trim_i8_encode(
            sk + u, PQCLEAN_FALCON512PADDED_CLEAN_CRYPTO_SECRETKEYBYTES - u,
            f, 9, PQCLEAN_FALCON512PADDED_CLEAN_max_fg_bits[9]);
    if (v == 0) {
        return -1;
    }
    u += v;
    v = PQCLEAN_FALCON512PADDED_CLEAN_trim_i8_encode(
            sk + u, PQCLEAN_FALCON512PADDED_CLEAN_CRYPTO_SECRETKEYBYTES - u,
            g, 9, PQCLEAN_FALCON512PADDED_CLEAN_max_fg_bits[9]);
    if (v == 0) {
        return -1;
    }
    u += v;
    ...
    u += v;
    if (u != PQCLEAN_FALCON512PADDED_CLEAN_CRYPTO_SECRETKEYBYTES) {
        return -1;
    }
     ...
     */
    pk[0] = 0x00 + 9;
    v = PQCLEAN_FALCON512PADDED_CLEAN_modq_encode(
            pk + 1, PQCLEAN_FALCON512PADDED_CLEAN_CRYPTO_PUBLICKEYBYTES - 1,
            h, 9);
    if (v != PQCLEAN_FALCON512PADDED_CLEAN_CRYPTO_PUBLICKEYBYTES - 1) {
        return -1;
    }
...
do_sign(uint8_t *nonce, uint8_t *sigbuf, size_t sigbuflen,
    ...
    u = 1;
    v = PQCLEAN_FALCON512PADDED_CLEAN_trim_i8_decode(
            f, 9, PQCLEAN_FALCON512PADDED_CLEAN_max_fg_bits[9],
            sk + u, PQCLEAN_FALCON512PADDED_CLEAN_CRYPTO_SECRETKEYBYTES - u);
    if (v == 0) {
        return -1;
    ...
    if (!PQCLEAN_FALCON512PADDED_CLEAN_complete_private(G, f, g, F, 9, tmp.b)) {
        return -1;
    }
    ...
    PQCLEAN_FALCON512PADDED_CLEAN_hash_to_point_ct(&sc, r.hm, 9, tmp.b);
     ...
     */
    for (;;) {
        PQCLEAN_FALCON512PADDED_CLEAN_sign_dyn(r.sig, &sc, f, g, F, G, r.hm, 9, tmp.b);
        v = PQCLEAN_FALCON512PADDED_CLEAN_comp_encode(sigbuf, sigbuflen, r.sig, 9);
        if (v != 0) {
            inner_shake256_ctx_release(&sc);
...
do_verify(
    ...
    PQCLEAN_FALCON512PADDED_CLEAN_to_ntt_monty(h, 9);
    ...
    v = PQCLEAN_FALCON512PADDED_CLEAN_comp_decode(sig, 9, sigbuf, sigbuflen);
    if (v == 0) {
        return -1;
    }
    if (v != sigbuflen) {
    ...
    inner_shake256_inject(&sc, m, mlen);
    inner_shake256_flip(&sc);
    PQCLEAN_FALCON512PADDED_CLEAN_hash_to_point_ct(&sc, hm, 9, tmp.b);
    inner_shake256_ctx_release(&sc);

    /*
     * Verify signature.
     */
    if (!PQCLEAN_FALCON512PADDED_CLEAN_verify_raw(hm, sig, h, 9, tmp.b)) {
        return -1;
    }
    return 0;
}
...
/* see api.h */
int
PQCLEAN_FALCON512PADDED_CLEAN_crypto_sign_signature(
    uint8_t *sig, size_t *siglen,
    const uint8_t *m, size_t mlen, const uint8_t *sk) {
    size_t vlen;

    vlen = PQCLEAN_FALCON512PADDED_CLEAN_CRYPTO_BYTES - NONCELEN - 1;
...
/* see api.h */
int
PQCLEAN_FALCON512PADDED_CLEAN_crypto_sign_verify(
...
/* see api.h */
int
PQCLEAN_FALCON512PADDED_CLEAN_crypto_sign(
...
/* see api.h */
int
PQCLEAN_FALCON512PADDED_CLEAN_crypto_sign_open(
    uint8_t *m, size_t *mlen,
    const uint8_t *sm, size_t smlen, const uint8_t *pk) {
    const uint8_t *sigbuf;
    size_t pmlen, sigbuflen;

    if (smlen < PQCLEAN_FALCON512PADDED_CLEAN_CRYPTO_BYTES) {
        return -1;
    }
    sigbuflen = PQCLEAN_FALCON512PADDED_CLEAN_CRYPTO_BYTES - NONCELEN - 1;
    pmlen = smlen - PQCLEAN_FALCON512PADDED_CLEAN_CRYPTO_BYTES;
    if (sm[0] != 0x30 + 9) {
        return -1;
    }
    sigbuf = sm + 1 + NONCELEN;
    ...
    if (do_verify(sm + 1, sigbuf, sigbuflen,
                  sm + PQCLEAN_FALCON512PADDED_CLEAN_CRYPTO_BYTES, pmlen, pk) < 0) {
        return -1;
    }
     ...
     * overlaps.
     */
    memmove(m, sm + PQCLEAN_FALCON512PADDED_CLEAN_CRYPTO_BYTES, pmlen);
    *mlen = pmlen;
    return 0;
}
