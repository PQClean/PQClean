...
void PQCLEAN_GEMSS192_AVX2_precSignHFESeed(secret_key_HFE *sk_HFE, const UINT *sk) {
    Tnv_gf2 L, U;
    ...
    PQCLEAN_GEMSS192_AVX2_cleanMonicHFEv_gf2nx(sk_HFE->F_HFEv);
    ...
    PQCLEAN_GEMSS192_AVX2_cleanLowerMatrixnv(L);
    PQCLEAN_GEMSS192_AVX2_cleanLowerMatrixnv(U);

    /* Generate S^(-1) = L*U */
    PQCLEAN_GEMSS192_AVX2_mulMatricesLU_gf2_nv(sk_HFE->S, L, U);
    ...
    PQCLEAN_GEMSS192_AVX2_cleanLowerMatrixn(L);
    PQCLEAN_GEMSS192_AVX2_cleanLowerMatrixn(U);

    /* Generate T^(-1) = L*U */
    PQCLEAN_GEMSS192_AVX2_mulMatricesLU_gf2_n(sk_HFE->T, L, U);

}


/* Precomputation for one secret-key */
int PQCLEAN_GEMSS192_AVX2_precSignHFE(secret_key_HFE *sk_HFE, const UINT **linear_coefs,
                                      const UINT *sk) {
    cst_sparse_monic_gf2nx F_HFEv;
    UINT *F;
    unsigned int i;

    PQCLEAN_GEMSS192_AVX2_precSignHFESeed(sk_HFE, sk);

    PQCLEAN_GEMSS192_AVX2_initListDifferences_gf2nx(sk_HFE->F_struct.L);
...
int PQCLEAN_GEMSS192_AVX2_signHFE_FeistelPatarin(unsigned char *sm8,
    ...
    nb_root = PQCLEAN_GEMSS192_AVX2_precSignHFE(&sk_HFE, linear_coefs, sk);
    if (nb_root) {
        /* Error from malloc */
        return nb_root;
    }
    ...
    for (k = 1; k <= NB_ITE; ++k) {
        ...
        do {
            ...
            nb_root = PQCLEAN_GEMSS192_AVX2_chooseRootHFE_gf2nx(DR, sk_HFE.F_struct, U);
            if (!nb_root) {
