 ...
 * @remark  Constant-time implementation.
 */
void PQCLEAN_GEMSS192_AVX2_convMQS_one_eq_to_hybrid_rep8_comp_gf2(uint8_t *pk2,
        const uint8_t *pk) {
    unsigned int i;
    unsigned int j;
    ...
    for (j = 0; j < HFEmr8; ++j) {
        PQCLEAN_GEMSS192_AVX2_convMQ_UL_gf2(pk2 + ACCESS_last_equations8 + j * NB_BYTES_EQUATION,
 ...
 * PQCLEAN_GEMSS192_AVX2_convMQS_one_eq_to_hybrid_rep8_comp_gf2 when N mod 8 = 0 (the last byte of
 * each equation is already filled) or when m mod 8 = 1 (the equation is
 * already compressed).
 * @remark  Constant-time implementation.
 */
void PQCLEAN_GEMSS192_AVX2_convMQS_one_eq_to_hybrid_rep8_uncomp_gf2(uint8_t *pk2,
        const uint8_t *pk) {
    unsigned int i;
    unsigned int j;
    ...
    val = PQCLEAN_GEMSS192_AVX2_convMQ_last_UL_gf2(pk2 + ACCESS_last_equations8
