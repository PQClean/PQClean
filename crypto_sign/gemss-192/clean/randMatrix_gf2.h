...
#define GEN_LU_Tn(L,U) \
    PQCLEAN_GEMSS192_CLEAN_genLowerMatrixn(L);\
    /* Generate the transpose of U */\
    PQCLEAN_GEMSS192_CLEAN_genLowerMatrixn(U);

#define GEN_LU_Tnv(L,U) \
    PQCLEAN_GEMSS192_CLEAN_genLowerMatrixnv(L);\
    /* Generate the transpose of U */\
    PQCLEAN_GEMSS192_CLEAN_genLowerMatrixnv(U);
...
#define randInvMatrixLU_gf2_n(S,L,U) \
    GEN_LU_Tn(L,U);\
    PQCLEAN_GEMSS192_CLEAN_mulMatricesLU_gf2_n(S,L,U);

#define randInvMatrixLU_gf2_nv(S,L,U) \
    GEN_LU_Tnv(L,U);\
    PQCLEAN_GEMSS192_CLEAN_mulMatricesLU_gf2_nv(S,L,U);
