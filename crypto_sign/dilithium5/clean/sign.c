...
**************************************************/
int PQCLEAN_DILITHIUM5_CLEAN_crypto_sign_signature_ctx(uint8_t *sig,
        size_t *siglen,
        const uint8_t *m,
        size_t mlen,
        const uint8_t *ctx,
...
* Returns 0 (success) or -1 (context string too long)
**************************************************/
int PQCLEAN_DILITHIUM5_CLEAN_crypto_sign_ctx(uint8_t *sm,
        size_t *smlen,
        const uint8_t *m,
        size_t mlen,
        const uint8_t *ctx,
    ...
    ret = PQCLEAN_DILITHIUM5_CLEAN_crypto_sign_signature_ctx(sm, smlen, sm + PQCLEAN_DILITHIUM5_CLEAN_CRYPTO_BYTES, mlen, ctx, ctxlen, sk);
    *smlen += mlen;
    return ret;
}
...
**************************************************/
int PQCLEAN_DILITHIUM5_CLEAN_crypto_sign_verify_ctx(const uint8_t *sig,
        size_t siglen,
        const uint8_t *m,
        size_t mlen,
        const uint8_t *ctx,
    ...
    if (ctxlen > 255 || siglen != PQCLEAN_DILITHIUM5_CLEAN_CRYPTO_BYTES) {
        return -1;
    }
    ...
    shake256(mu, TRBYTES, pk, PQCLEAN_DILITHIUM5_CLEAN_CRYPTO_PUBLICKEYBYTES);
    shake256_inc_init(&state);
...
**************************************************/
int PQCLEAN_DILITHIUM5_CLEAN_crypto_sign_open_ctx(uint8_t *m,
        size_t *mlen,
        const uint8_t *sm,
        size_t smlen,
        const uint8_t *ctx,
    ...
    *mlen = smlen - PQCLEAN_DILITHIUM5_CLEAN_CRYPTO_BYTES;
    if (PQCLEAN_DILITHIUM5_CLEAN_crypto_sign_verify_ctx(sm, PQCLEAN_DILITHIUM5_CLEAN_CRYPTO_BYTES, sm + PQCLEAN_DILITHIUM5_CLEAN_CRYPTO_BYTES, *mlen, ctx, ctxlen, pk)) {
        goto badsig;
    } else {
