...
#define VAL_BITS_M (((45)<(8-HFEmr8))?(HFEDELTA+HFEv):(8-HFEmr8))
...
static void compress_signHFE(unsigned char *sm8, const UINT *sm) {
    ...
    for (k1 = 1; k1 < NB_ITE; ++k1) {
        ...
        val_n = ((45) < ((8 - (nb_bits & 7U)) & 7U)) ? (HFEDELTA + HFEv)
                : ((8 - (nb_bits & 7U)) & 7U);
...
void PQCLEAN_GEMSSBLUE192_CLEAN_precSignHFESeed(secret_key_HFE *sk_HFE, const UINT *sk) {
    Tnv_gf2 L, U;
    ...
    PQCLEAN_GEMSSBLUE192_CLEAN_cleanMonicHFEv_gf2nx(sk_HFE->F_HFEv);
    ...
    PQCLEAN_GEMSSBLUE192_CLEAN_cleanLowerMatrixnv(L);
    PQCLEAN_GEMSSBLUE192_CLEAN_cleanLowerMatrixnv(U);

    /* Generate S^(-1) = L*U */
    PQCLEAN_GEMSSBLUE192_CLEAN_mulMatricesLU_gf2_nv(sk_HFE->S, L, U);
    ...
    PQCLEAN_GEMSSBLUE192_CLEAN_cleanLowerMatrixn(L);
    PQCLEAN_GEMSSBLUE192_CLEAN_cleanLowerMatrixn(U);

    /* Generate T^(-1) = L*U */
    PQCLEAN_GEMSSBLUE192_CLEAN_mulMatricesLU_gf2_n(sk_HFE->T, L, U);

}


/* Precomputation for one secret-key */
int PQCLEAN_GEMSSBLUE192_CLEAN_precSignHFE(secret_key_HFE *sk_HFE, const UINT **linear_coefs,
        const UINT *sk) {
    cst_sparse_monic_gf2nx F_HFEv;
    UINT *F;
    unsigned int i;

    PQCLEAN_GEMSSBLUE192_CLEAN_precSignHFESeed(sk_HFE, sk);

    PQCLEAN_GEMSSBLUE192_CLEAN_initListDifferences_gf2nx(sk_HFE->F_struct.L);
...
int PQCLEAN_GEMSSBLUE192_CLEAN_signHFE_FeistelPatarin(unsigned char *sm8,
    ...
    nb_root = PQCLEAN_GEMSSBLUE192_CLEAN_precSignHFE(&sk_HFE, linear_coefs, sk);
    if (nb_root) {
        /* Error from malloc */
        return nb_root;
    }
    ...
    for (k = 1; k <= NB_ITE; ++k) {
        ...
        do {
            ...
            nb_root = PQCLEAN_GEMSSBLUE192_CLEAN_chooseRootHFE_gf2nx(DR, sk_HFE.F_struct, U);
            if (!nb_root) {
