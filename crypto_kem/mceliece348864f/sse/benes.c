...
void PQCLEAN_MCELIECE348864F_SSE_load_bits(uint64_t out[][32], const unsigned char *bits) {
    int i, low, block = 0;
    ...
    for (low = 0; low <= 5; low++) {
        for (i = 0; i < 64; i++) {
            cond[i] = PQCLEAN_MCELIECE348864F_SSE_load4(bits + block * 256 + i * 4);
        }
        PQCLEAN_MCELIECE348864F_SSE_transpose_64x64(cond);
    ...
    for (low = 0; low <= 5; low++) {
        for (i = 0; i < 32; i++) {
            out[ block ][i] = PQCLEAN_MCELIECE348864F_SSE_load8(bits + block * 256 + i * 8);
        }
        block++;
    }
...
void PQCLEAN_MCELIECE348864F_SSE_benes(uint64_t *r, uint64_t cond[][32], int rev) {
    int block, inc;
    ...
    PQCLEAN_MCELIECE348864F_SSE_transpose_64x64(bs);
