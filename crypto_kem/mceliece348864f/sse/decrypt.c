...
static void scaling(vec128 out[][GFBITS], vec128 inv[][GFBITS], const unsigned char *sk, vec128 *recv) {
    ...
    for (i = 0; i < 32; i++) {
        PQCLEAN_MCELIECE348864F_SSE_vec128_sq(eval[i], eval[i]);
    }
    ...
    PQCLEAN_MCELIECE348864F_SSE_vec128_inv(tmp, inv[31]);
...
static void preprocess(vec128 *recv, const unsigned char *s) {
    ...
    for (i = 0; i < 32; i++) {
        recv[i] = PQCLEAN_MCELIECE348864F_SSE_load16(r + i * 16);
    }
}
...
static void postprocess(unsigned char *e, vec128 *err) {
    ...
    for (i = 0; i < 32; i++) {
        ...
        PQCLEAN_MCELIECE348864F_SSE_store8(error8 + i * 16 + 0, v[0]);
        PQCLEAN_MCELIECE348864F_SSE_store8(error8 + i * 16 + 8, v[1]);
    }
...
int PQCLEAN_MCELIECE348864F_SSE_decrypt(unsigned char *e, const unsigned char *sk, const unsigned char *c) {
    int i;
    ...
    PQCLEAN_MCELIECE348864F_SSE_load_bits(bits_int, sk + IRR_BYTES);
    PQCLEAN_MCELIECE348864F_SSE_benes((uint64_t *) recv, bits_int, 1);

    scaling(scaled, inv, sk, recv);

    PQCLEAN_MCELIECE348864F_SSE_fft_tr(s_priv, scaled);

    PQCLEAN_MCELIECE348864F_SSE_bm(locator, s_priv);

    PQCLEAN_MCELIECE348864F_SSE_fft(eval, locator);
    ...
    PQCLEAN_MCELIECE348864F_SSE_fft_tr(s_priv_cmp, scaled);

    check_synd = synd_cmp(s_priv, s_priv_cmp);

    //

    PQCLEAN_MCELIECE348864F_SSE_benes((uint64_t *) error, bits_int, 0);
