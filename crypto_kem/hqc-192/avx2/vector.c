 ...
 */
void PQCLEAN_HQC192_AVX2_vect_set_random_fixed_weight(seedexpander_state *ctx, __m256i *v256, uint16_t weight) {
    ...
    PQCLEAN_HQC192_AVX2_seedexpander(ctx, (uint8_t *)&rand_u32, 4 * weight);
 ...
 * array of bytes using the PQCLEAN_HQC192_AVX2_seedexpander function, and drop the extra bits using a mask.
 *
 * @param[in] v Pointer to an array
 * @param[in] ctx Pointer to the context of the seed expander
 */
void PQCLEAN_HQC192_AVX2_vect_set_random(seedexpander_state *ctx, uint64_t *v) {
    uint8_t rand_bytes[VEC_N_SIZE_BYTES] = {0};

    PQCLEAN_HQC192_AVX2_seedexpander(ctx, rand_bytes, VEC_N_SIZE_BYTES);
 ...
 */
void PQCLEAN_HQC192_AVX2_vect_set_random_from_prng(uint64_t *v, uint32_t size_v) {
    uint8_t rand_bytes [32] = {0}; // set to the maximum possible size - 256 bits

    PQCLEAN_HQC192_AVX2_shake_prng(rand_bytes, size_v << 3);
 ...
 */
void PQCLEAN_HQC192_AVX2_vect_print(const uint64_t *v, const uint32_t size) {
 ...
 */
void PQCLEAN_HQC192_AVX2_vect_print_sparse(const uint32_t *v, const uint16_t weight) {
