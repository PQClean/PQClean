...
void PQCLEAN_HQC192_CLEAN_hqc_pke_encrypt(uint64_t *u, uint64_t *v, uint8_t *m, unsigned char *theta, const unsigned char *pk) {
    ...
    PQCLEAN_HQC192_CLEAN_code_encode((uint8_t *)v, m);
    PQCLEAN_HQC192_CLEAN_load8_arr(v, VEC_N1N2_SIZE_64, (uint8_t *)v, VEC_N1N2_SIZE_BYTES);
 ...
 */
void PQCLEAN_HQC192_CLEAN_hqc_pke_decrypt(uint8_t *m, const uint64_t *u, const uint64_t *v, const unsigned char *sk) {
    ...
    PQCLEAN_HQC192_CLEAN_hqc_secret_key_from_string(tmp1, y, pk, sk);
    ...
    PQCLEAN_HQC192_CLEAN_store8_arr((uint8_t *)tmp1, VEC_N_SIZE_BYTES, tmp2, VEC_N_SIZE_64);
    PQCLEAN_HQC192_CLEAN_code_decode(m, (uint8_t *)tmp1);
}
