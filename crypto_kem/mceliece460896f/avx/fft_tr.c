...
#include "consts.inc"
    ...
    for (i = 4; i >= 0; i--) {
        ...
        for (j = 0; j < 32; j += 2 * s) {
            for (k = j; k < j + s; k++) {
                PQCLEAN_MCELIECE460896F_AVX_vec256_ama_asm(in[k], in[k + s], consts[ consts_ptr + (k - j) ]);
            }
        }
    ...
    for (k = 0; k < 32; k += 2) {
        ...
        PQCLEAN_MCELIECE460896F_AVX_vec256_ama_asm(t0, t1, consts[1]);
        ...
        PQCLEAN_MCELIECE460896F_AVX_vec256_ama_asm(t0, t1, consts[0]);
