...
void PQCLEAN_HQC128_AVX2_hqc_pke_keygen(unsigned char *pk, unsigned char *sk) {
    ...
    PQCLEAN_HQC128_AVX2_shake_prng(sk_seed, SEED_BYTES);
    PQCLEAN_HQC128_AVX2_shake_prng(sigma, VEC_K_SIZE_BYTES);
    PQCLEAN_HQC128_AVX2_seedexpander_init(&sk_seedexpander, sk_seed, SEED_BYTES);

    PQCLEAN_HQC128_AVX2_shake_prng(pk_seed, SEED_BYTES);
    PQCLEAN_HQC128_AVX2_seedexpander_init(&pk_seedexpander, pk_seed, SEED_BYTES);

    // Compute secret key
    PQCLEAN_HQC128_AVX2_vect_set_random_fixed_weight(&sk_seedexpander, x_256, PARAM_OMEGA);
    PQCLEAN_HQC128_AVX2_vect_set_random_fixed_weight(&sk_seedexpander, y_256, PARAM_OMEGA);

    // Compute public key
    PQCLEAN_HQC128_AVX2_vect_set_random(&pk_seedexpander, (uint64_t *) h_256);
    PQCLEAN_HQC128_AVX2_vect_mul(tmp_256, y_256, h_256);
    PQCLEAN_HQC128_AVX2_vect_add(s, (uint64_t *) x_256, (uint64_t *) tmp_256, VEC_N_256_SIZE_64);

    // Parse keys to string
    PQCLEAN_HQC128_AVX2_hqc_public_key_to_string(pk, pk_seed, s);
    PQCLEAN_HQC128_AVX2_hqc_secret_key_to_string(sk, sk_seed, sigma, pk);
...
void PQCLEAN_HQC128_AVX2_hqc_pke_encrypt(uint64_t *u, uint64_t *v, uint64_t *m, unsigned char *theta, const unsigned char *pk) {
    seedexpander_state PQCLEAN_HQC128_AVX2_seedexpander;
    static __m256i h_256[VEC_N_256_SIZE_64 >> 2];
    ...
    PQCLEAN_HQC128_AVX2_seedexpander_init(&PQCLEAN_HQC128_AVX2_seedexpander, theta, SEED_BYTES);

    // Retrieve h and s from public key
    PQCLEAN_HQC128_AVX2_hqc_public_key_from_string((uint64_t *) h_256, (uint64_t *) s_256, pk);

    // Generate r1, r2 and e
    PQCLEAN_HQC128_AVX2_vect_set_random_fixed_weight(&PQCLEAN_HQC128_AVX2_seedexpander, r1_256, PARAM_OMEGA_R);
    PQCLEAN_HQC128_AVX2_vect_set_random_fixed_weight(&PQCLEAN_HQC128_AVX2_seedexpander, r2_256, PARAM_OMEGA_R);
    PQCLEAN_HQC128_AVX2_vect_set_random_fixed_weight(&PQCLEAN_HQC128_AVX2_seedexpander, e_256, PARAM_OMEGA_E);

    // Compute u = r1 + r2.h
    PQCLEAN_HQC128_AVX2_vect_mul(tmp1_256, r2_256, h_256);
    PQCLEAN_HQC128_AVX2_vect_add(u, (uint64_t *) r1_256, (uint64_t *) tmp1_256, VEC_N_256_SIZE_64);

    // Compute v = m.G by encoding the message
    PQCLEAN_HQC128_AVX2_code_encode(v, m);
    PQCLEAN_HQC128_AVX2_vect_resize((uint64_t *) tmp2_256, PARAM_N, v, PARAM_N1N2);

    // Compute v = m.G + s.r2 + e
    PQCLEAN_HQC128_AVX2_vect_mul(tmp3_256, r2_256, s_256);
    PQCLEAN_HQC128_AVX2_vect_add(tmp4, (uint64_t *) e_256, (uint64_t *) tmp3_256, VEC_N_256_SIZE_64);
    PQCLEAN_HQC128_AVX2_vect_add((uint64_t *) tmp3_256, (uint64_t *) tmp2_256, tmp4, VEC_N_256_SIZE_64);
    PQCLEAN_HQC128_AVX2_vect_resize(v, PARAM_N1N2, (uint64_t *) tmp3_256, PARAM_N);
 ...
 * @returns 0
 */
uint8_t PQCLEAN_HQC128_AVX2_hqc_pke_decrypt(uint64_t *m, uint8_t *sigma, const __m256i *u_256, const uint64_t *v, const uint8_t *sk) {
    ...
    PQCLEAN_HQC128_AVX2_hqc_secret_key_from_string(x_256, y_256, sigma, pk, sk);

    // Compute v - u.y
    PQCLEAN_HQC128_AVX2_vect_resize(tmp1, PARAM_N, v, PARAM_N1N2);
    PQCLEAN_HQC128_AVX2_vect_mul(tmp3_256, y_256, u_256);
    PQCLEAN_HQC128_AVX2_vect_add(tmp2, tmp1, (uint64_t *) tmp3_256, VEC_N_256_SIZE_64);
