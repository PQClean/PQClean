...
static void de_bitslicing(uint64_t *out, vec128 in[][GFBITS]) {
    ...
    for (i = 0; i < 64; i++) {
        for (j = GFBITS - 1; j >= 0; j--) {
            u = PQCLEAN_MCELIECE460896_SSE_vec128_extract(in[i][j], 0);
            for (r = 0; r < 64; r++) {
                out[i * 128 + 0 * 64 + r] <<= 1;
                out[i * 128 + 0 * 64 + r] |= (u >> r) & 1;
            }
            u = PQCLEAN_MCELIECE460896_SSE_vec128_extract(in[i][j], 1);
            for (r = 0; r < 64; r++) {
                out[i * 128 + 1 * 64 + r] <<= 1;
...
static void to_bitslicing_2x(vec128 out0[][GFBITS], vec128 out1[][GFBITS], const uint64_t *in) {
    ...
    for (i = 0; i < 64; i++) {
        for (j = GFBITS - 1; j >= 0; j--) {
            ...
            out1[i][j] = PQCLEAN_MCELIECE460896_SSE_vec128_set2x(u[0], u[1]);
        }
        ...
        for (j = GFBITS - 1; j >= 0; j--) {
            ...
            out0[i][GFBITS - 1 - j] = PQCLEAN_MCELIECE460896_SSE_vec128_set2x(u[0], u[1]);
        }
    }
}
...
int PQCLEAN_MCELIECE460896_SSE_pk_gen(unsigned char *pk, uint32_t *perm, const uint8_t *sk) {
    ...
    PQCLEAN_MCELIECE460896_SSE_vec128_copy(prod[0], eval[0]);

    for (i = 1; i < 64; i++) {
        PQCLEAN_MCELIECE460896_SSE_vec128_mul(prod[i], prod[i - 1], eval[i]);
    }

    PQCLEAN_MCELIECE460896_SSE_vec128_inv(tmp, prod[63]);

    for (i = 62; i >= 0; i--) {
        PQCLEAN_MCELIECE460896_SSE_vec128_mul(prod[i + 1], prod[i], tmp);
        PQCLEAN_MCELIECE460896_SSE_vec128_mul(tmp, tmp, eval[i + 1]);
    }

    PQCLEAN_MCELIECE460896_SSE_vec128_copy(prod[0], tmp);
    ...
    PQCLEAN_MCELIECE460896_SSE_sort_63b(1 << GFBITS, list);
    ...
    for (j = 0; j < NBLOCKS2_I; j++) {
        for (k = 0; k < GFBITS; k++) {
            mat[ k ][ 2 * j + 0 ] = PQCLEAN_MCELIECE460896_SSE_vec128_extract(prod[ j ][ k ], 0);
            mat[ k ][ 2 * j + 1 ] = PQCLEAN_MCELIECE460896_SSE_vec128_extract(prod[ j ][ k ], 1);
        }
    }

    for (i = 1; i < SYS_T; i++) {
        for (j = 0; j < NBLOCKS2_I; j++) {
            PQCLEAN_MCELIECE460896_SSE_vec128_mul(prod[j], prod[j], consts[j]);

            for (k = 0; k < GFBITS; k++) {
                mat[ i * GFBITS + k ][ 2 * j + 0 ] = PQCLEAN_MCELIECE460896_SSE_vec128_extract(prod[ j ][ k ], 0);
                mat[ i * GFBITS + k ][ 2 * j + 1 ] = PQCLEAN_MCELIECE460896_SSE_vec128_extract(prod[ j ][ k ], 1);
            }
        }
    }
    ...
    for (row = 0; row < PK_NROWS; row++) {
        ...
        for (k = block_idx; k < NBLOCKS1_H - 1; k++) {
            ...
            PQCLEAN_MCELIECE460896_SSE_store8(pk, one_row[k]);
            pk += 8;
        }

        one_row[k] >>= tail;
        PQCLEAN_MCELIECE460896_SSE_store_i(pk, one_row[k], PK_ROW_BYTES % 8);
