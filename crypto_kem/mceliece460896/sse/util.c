...
void PQCLEAN_MCELIECE460896_SSE_irr_load(vec128 *out, const unsigned char *in) {
    ...
    for (i = 0; i < SYS_T; i++) {
        irr[i] = PQCLEAN_MCELIECE460896_SSE_load2(in + i * 2);
        irr[i] &= GFMASK;
    }
...
vec128 PQCLEAN_MCELIECE460896_SSE_load16(const unsigned char *in) {
    return vec128_set2x( PQCLEAN_MCELIECE460896_SSE_load8(in), PQCLEAN_MCELIECE460896_SSE_load8(in + 8) );
}

void PQCLEAN_MCELIECE460896_SSE_store16(unsigned char *out, vec128 in) {
    PQCLEAN_MCELIECE460896_SSE_store8(out + 0, vec128_extract(in, 0));
    PQCLEAN_MCELIECE460896_SSE_store8(out + 8, vec128_extract(in, 1));
}
