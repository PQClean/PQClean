...
gf PQCLEAN_MCELIECE460896_SSE_gf_inv(gf in) {
    return PQCLEAN_MCELIECE460896_SSE_gf_frac(in, ((gf) 1));
}
...
void PQCLEAN_MCELIECE460896_SSE_GF_mul(gf *out, const gf *in0, const gf *in1) {
    ...
    for (i = 0; i < 96; i++) {
        for (j = 0; j < 96; j++) {
            prod[i + j] ^= PQCLEAN_MCELIECE460896_SSE_gf_mul(in0[i], in1[j]);
        }
    }
    ...
    for (i = 190; i >= 96; i--) {
        prod[i - 85] ^= PQCLEAN_MCELIECE460896_SSE_gf_mul(prod[i], (gf) 714);
        prod[i - 91] ^= PQCLEAN_MCELIECE460896_SSE_gf_mul(prod[i], (gf) 5296);
        prod[i - 92] ^= PQCLEAN_MCELIECE460896_SSE_gf_mul(prod[i], (gf) 728);
        prod[i - 96] ^= PQCLEAN_MCELIECE460896_SSE_gf_mul(prod[i], (gf) 5881);
    }
