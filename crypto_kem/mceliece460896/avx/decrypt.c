...
static void scaling(vec256 out[][GFBITS], vec256 inv[][GFBITS], const unsigned char *sk, vec256 *recv) {
    ...
    PQCLEAN_MCELIECE460896_AVX_irr_load(sk_int, sk);

    PQCLEAN_MCELIECE460896_AVX_fft(eval, sk_int);

    for (i = 0; i < 32; i++) {
        PQCLEAN_MCELIECE460896_AVX_vec256_sq(eval[i], eval[i]);
    }
    ...
    PQCLEAN_MCELIECE460896_AVX_vec256_inv(tmp, inv[31]);
...
static void preprocess(vec128 *recv, const unsigned char *s) {
    ...
    for (i = 0; i < 64; i++) {
        recv[i] = PQCLEAN_MCELIECE460896_AVX_load16(r + i * 16);
    }
}
...
static void postprocess(unsigned char *e, vec128 *err) {
    ...
    for (i = 0; i < 64; i++) {
        ...
        PQCLEAN_MCELIECE460896_AVX_store8(error8 + i * 16 + 0, v[0]);
        PQCLEAN_MCELIECE460896_AVX_store8(error8 + i * 16 + 8, v[1]);
    }
...
int PQCLEAN_MCELIECE460896_AVX_decrypt(unsigned char *e, const unsigned char *sk, const unsigned char *c) {
    ...
    PQCLEAN_MCELIECE460896_AVX_load_bits(bits_int, sk + IRR_BYTES);
    PQCLEAN_MCELIECE460896_AVX_benes(recv128, bits_int, 1);

    reformat_128to256(recv256, recv128);

    scaling(scaled, inv, sk, recv256);
    PQCLEAN_MCELIECE460896_AVX_fft_tr(s_priv, scaled);
    PQCLEAN_MCELIECE460896_AVX_bm(locator, s_priv);

    PQCLEAN_MCELIECE460896_AVX_fft(eval, locator);
    ...
    PQCLEAN_MCELIECE460896_AVX_fft_tr(s_priv_cmp, scaled);
    ...
    PQCLEAN_MCELIECE460896_AVX_benes(error128, bits_int, 0);
