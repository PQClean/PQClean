...
static void Short_random(small *out) {
    ...
    PQCLEAN_SNTRUP1013_AVX2_crypto_sort_uint32(L, ppadsort);
    for (i = 0; i < p; ++i) {
        out[i] = (small) ((L[i] & 3) - 1);
    }
}
...
int PQCLEAN_SNTRUP1013_AVX2_crypto_kem_keypair(uint8_t *pk, uint8_t *sk) {
    small g[p];
    for (;;) {
        Small_random(g);
        {
...
int PQCLEAN_SNTRUP1013_AVX2_crypto_kem_enc(uint8_t *c, uint8_t *k, const uint8_t *pk) {
...
int PQCLEAN_SNTRUP1013_AVX2_crypto_kem_dec(uint8_t *k, const uint8_t *c, const uint8_t *sk) {
