...
(
    ...
    while (1) {
        rp = r;
        PQCLEAN_MCELIECE6688128F_CLEAN_aes256ctr(r, sizeof(r), nonce, seed);
        memcpy(seed, &r[ sizeof(r) - 32 ], 32);

        for (i = 0; i < SYS_T; i++) {
            f[i] = PQCLEAN_MCELIECE6688128F_CLEAN_load2(rp + i * 2);
        }
        rp += sizeof(f);
        if (PQCLEAN_MCELIECE6688128F_CLEAN_genpoly_gen(irr, f)) {
            continue;
        }

        for (i = 0; i < (1 << GFBITS); i++) {
            perm[i] = PQCLEAN_MCELIECE6688128F_CLEAN_load4(rp + i * 4);
        }
        rp += sizeof(perm);
        if (PQCLEAN_MCELIECE6688128F_CLEAN_perm_check(perm)) {
            continue;
        }
        ...
        PQCLEAN_MCELIECE6688128F_CLEAN_controlbits(sk + SYS_N / 8 + IRR_BYTES, perm);
