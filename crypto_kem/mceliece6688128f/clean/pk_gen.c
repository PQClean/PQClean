...
static int mov_columns(uint8_t mat[][ SYS_N / 8 ], uint32_t *perm) {
    ...
    for (i = 0; i < 32; i++) {
        buf[i] = PQCLEAN_MCELIECE6688128F_CLEAN_load8( &mat[ row + i ][ block_idx ] );
    }
    ...
    for (i = 0; i < GFBITS * SYS_T; i += 64) {
        for (j = 0; j < min(64, GFBITS * SYS_T - i); j++) {
            buf[j] = PQCLEAN_MCELIECE6688128F_CLEAN_load8( &mat[ i + j ][ block_idx ] );
        }
        ...
        for (j = 0; j < min(64, GFBITS * SYS_T - i); j++) {
            PQCLEAN_MCELIECE6688128F_CLEAN_store8( &mat[ i + j ][ block_idx ], buf[j] );
        }
    }
...
int PQCLEAN_MCELIECE6688128F_CLEAN_pk_gen(unsigned char *pk, unsigned char *sk, uint32_t *perm) {
    int i, j, k;
    int row, c;
    ...
    PQCLEAN_MCELIECE6688128F_CLEAN_sort_63b(1 << GFBITS, buf);

    for (i = 0; i < (1 << GFBITS); i++) {
        perm[i] = buf[i] & GFMASK;
    }
    for (i = 0; i < SYS_N;         i++) {
        L[i] = PQCLEAN_MCELIECE6688128F_CLEAN_bitrev(perm[i]);
    }

    // filling the matrix

    PQCLEAN_MCELIECE6688128F_CLEAN_root(inv, g, L);

    for (i = 0; i < SYS_N; i++) {
        inv[i] = PQCLEAN_MCELIECE6688128F_CLEAN_gf_inv(inv[i]);
    }
    ...
    for (i = 0; i < SYS_T; i++) {
        ...
        for (j = 0; j < SYS_N; j++) {
            inv[j] = PQCLEAN_MCELIECE6688128F_CLEAN_gf_mul(inv[j], L[j]);
        }
