...
void PQCLEAN_MCELIECE6688128F_AVX_load_bits(vec128 bits_int[][32], const unsigned char *bits) {
    ...
    for (i = 0; i <= 5; i += 2) {
        for (j = 0; j < 64; j++) {
            buf[j] = vec128_set2x(PQCLEAN_MCELIECE6688128F_AVX_load8(ptr), PQCLEAN_MCELIECE6688128F_AVX_load8(ptr + 512));
            ptr += 8;
        }

        PQCLEAN_MCELIECE6688128F_AVX_transpose_64x128_sp( buf );
    ...
    for (i = 6; i <= 18; i++) {
        for (j = 0; j < 32; j++) {
            bits_int[i][j] = PQCLEAN_MCELIECE6688128F_AVX_load16(ptr);
            ptr += 16;
        }
    }
...
void PQCLEAN_MCELIECE6688128F_AVX_benes(vec128 *r, vec128 b[][32], int rev) {
    ...
    PQCLEAN_MCELIECE6688128F_AVX_transpose_64x128_sp( r );
