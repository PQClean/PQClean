...
int PQCLEAN_HQC256_AVX2_crypto_kem_enc(unsigned char *ct, unsigned char *ss, const unsigned char *pk) {
    ...
    // Computing m
    PQCLEAN_HQC256_AVX2_vect_set_random_from_prng((uint64_t *)m, VEC_K_SIZE_64);

    // Computing theta
    PQCLEAN_HQC256_AVX2_vect_set_random_from_prng(salt, SALT_SIZE_64);
    ...
    PQCLEAN_HQC256_AVX2_shake256_512_ds(&shake256state, theta, tmp, VEC_K_SIZE_BYTES + PUBLIC_KEY_BYTES + SALT_SIZE_BYTES, G_FCT_DOMAIN);

    // Encrypting m
    PQCLEAN_HQC256_AVX2_hqc_pke_encrypt(u, v, (uint64_t *)m, theta, pk);

    // Computing shared secret
    memcpy(mc, m, VEC_K_SIZE_BYTES);
    memcpy(mc + VEC_K_SIZE_BYTES, u, VEC_N_SIZE_BYTES);
    memcpy(mc + VEC_K_SIZE_BYTES + VEC_N_SIZE_BYTES, v, VEC_N1N2_SIZE_BYTES);
    PQCLEAN_HQC256_AVX2_shake256_512_ds(&shake256state, ss, mc, VEC_K_SIZE_BYTES + VEC_N_SIZE_BYTES + VEC_N1N2_SIZE_BYTES, K_FCT_DOMAIN);

    // Computing ciphertext
    PQCLEAN_HQC256_AVX2_hqc_ciphertext_to_string(ct, u, v, salt);
...
int PQCLEAN_HQC256_AVX2_crypto_kem_dec(unsigned char *ss, const unsigned char *ct, const unsigned char *sk) {
    ...
    PQCLEAN_HQC256_AVX2_hqc_ciphertext_from_string((uint64_t *) u_256, v, salt, ct);

    // Retrieving pk from sk
    memcpy(pk, sk + SEED_BYTES, PUBLIC_KEY_BYTES);

    // Decrypting
    result = PQCLEAN_HQC256_AVX2_hqc_pke_decrypt((uint64_t *)m, sigma, u_256, v, sk);
    ...
    // Encrypting m'
    PQCLEAN_HQC256_AVX2_hqc_pke_encrypt(u2, v2, (uint64_t *)m, theta, pk);

    // Check if c != c'
    result |= PQCLEAN_HQC256_AVX2_vect_compare((uint8_t *) u_256, (uint8_t *) u2, VEC_N_SIZE_BYTES);
