...
static void Short_fromlist(small *out, const uint32 *in) {
    ...
    PQCLEAN_NTRULPR1013_CLEAN_crypto_sort_uint32(L, ppadsort);
    for (i = 0; i < p; ++i) {
        out[i] = (small) ((L[i] & 3) - 1);
    }
}
...
static void Generator(Fq *G, const unsigned char *pk) {
    ...
    if (PQCLEAN_NTRULPR1013_CLEAN_crypto_stream_aes256ctr((unsigned char *) L, 4 * p, aes_nonce, pk) != 0) {
        abort();
    }
    crypto_decode_pxint32(L, (unsigned char *) L);
    for (i = 0; i < p; ++i) {
...
static void Hide(unsigned char *c, unsigned char *r_enc, const Inputs r, const unsigned char *pk, const unsigned char *cache) {
    ...
    Inputs_encode(r_enc + 1, r);
    {
        ...
        if (PQCLEAN_NTRULPR1013_CLEAN_crypto_stream_aes256ctr((unsigned char *) L, 4 * p, aes_nonce, h) != 0) {
            abort();
        }
        crypto_decode_pxint32(L, (unsigned char *) L);
        Short_fromlist(b, L);
...
int PQCLEAN_NTRULPR1013_CLEAN_crypto_kem_keypair(uint8_t *pk, uint8_t *sk) {
    Fq aG[p];
    int i;
...
int PQCLEAN_NTRULPR1013_CLEAN_crypto_kem_enc(uint8_t *c, uint8_t *k, const uint8_t *pk) {
    int i;
...
int PQCLEAN_NTRULPR1013_CLEAN_crypto_kem_dec(uint8_t *k, const uint8_t *c, const uint8_t *sk) {
