...
static void Short_fromlist(small *out, const uint32 *in) {
    ...
    PQCLEAN_NTRULPR1013_AVX2_crypto_sort_uint32(L, ppadsort);
    for (i = 0; i < p; ++i) {
        out[i] = (small) ((L[i] & 3) - 1);
    }
}
...
static void Generator(Fq *G, const unsigned char *pk) {
    ...
    if (PQCLEAN_NTRULPR1013_AVX2_crypto_stream_aes256ctr((unsigned char *) L, 4 * p, aes_nonce, pk) != 0) {
        abort();
    }
    crypto_decode_pxint32(L, (unsigned char *) L);
    for (i = 0; i < p; ++i) {
...
static void Hide(unsigned char *c, unsigned char *r_enc, const Inputs r, const unsigned char *pk, const unsigned char *cache) {
    ...
    Inputs_encode(r_enc + 1, r);
    {
        ...
        if (PQCLEAN_NTRULPR1013_AVX2_crypto_stream_aes256ctr((unsigned char *) L, 4 * p, aes_nonce, h) != 0) {
            abort();
        }
        crypto_decode_pxint32(L, (unsigned char *) L);
        Short_fromlist(b, L);
