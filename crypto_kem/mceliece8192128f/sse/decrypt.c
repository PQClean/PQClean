...
static void scaling(vec128 out[][GFBITS], vec128 inv[][GFBITS], const unsigned char *sk, vec128 *recv) {
    ...
    for (i = 0; i < 64; i++) {
        PQCLEAN_MCELIECE8192128F_SSE_vec128_sq(eval[i], eval[i]);
    }
    ...
    PQCLEAN_MCELIECE8192128F_SSE_vec128_inv(tmp, inv[63]);
...
static void preprocess(vec128 *recv, const unsigned char *s) {
    ...
    for (i = 0; i < SYND_BYTES / 16; i++) {
        recv[i] = PQCLEAN_MCELIECE8192128F_SSE_load16(s + i * 16);
    }
}
...
int PQCLEAN_MCELIECE8192128F_SSE_decrypt(unsigned char *e, const unsigned char *sk, const unsigned char *c) {
    ...
    PQCLEAN_MCELIECE8192128F_SSE_load_bits(bits_int, sk + IRR_BYTES);
    PQCLEAN_MCELIECE8192128F_SSE_benes(recv, bits_int, 1);

    scaling(scaled, inv, sk, recv);
    PQCLEAN_MCELIECE8192128F_SSE_fft_tr(s_priv, scaled);
    PQCLEAN_MCELIECE8192128F_SSE_bm(locator, s_priv);

    PQCLEAN_MCELIECE8192128F_SSE_fft(eval, locator);
    ...
    PQCLEAN_MCELIECE8192128F_SSE_fft_tr(s_priv_cmp, scaled);

    check_synd = synd_cmp(s_priv, s_priv_cmp);

    //

    PQCLEAN_MCELIECE8192128F_SSE_benes(error, bits_int, 0);

    for (i = 0; i < 64; i++) {
        PQCLEAN_MCELIECE8192128F_SSE_store16(e + i * 16, error[i]);
    }
