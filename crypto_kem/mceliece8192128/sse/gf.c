...
uint64_t PQCLEAN_MCELIECE8192128_SSE_gf_mul2(gf a, gf b0, gf b1) {
    int i;
...
gf PQCLEAN_MCELIECE8192128_SSE_gf_mul(gf in0, gf in1) {
    int i;
...
/* return num/den */
gf PQCLEAN_MCELIECE8192128_SSE_gf_frac(gf den, gf num) {
    gf tmp_11;
    gf tmp_1111;
    gf out;
...
/* return 1/den */
gf PQCLEAN_MCELIECE8192128_SSE_gf_inv(gf in) {
    return PQCLEAN_MCELIECE8192128_SSE_gf_frac(in, ((gf) 1));
}

/* check if a == 0 */
gf PQCLEAN_MCELIECE8192128_SSE_gf_iszero(gf a) {
    uint32_t t = a;
...
void PQCLEAN_MCELIECE8192128_SSE_GF_mul(gf *out, const gf *in0, const gf *in1) {
    int i, j;
    ...
    for (i = 0; i < 128; i++) {
        for (j = 0; j < 128; j++) {
            prod[i + j] ^= PQCLEAN_MCELIECE8192128_SSE_gf_mul(in0[i], in1[j]);
        }
    }
    ...
    for (i = 254; i >= 128; i--) {
        prod[i - 123] ^= PQCLEAN_MCELIECE8192128_SSE_gf_mul(prod[i], (gf) 7682);
        prod[i - 125] ^= PQCLEAN_MCELIECE8192128_SSE_gf_mul(prod[i], (gf) 2159);
        prod[i - 128] ^= PQCLEAN_MCELIECE8192128_SSE_gf_mul(prod[i], (gf) 3597);
    }
