...
/* check if a == 0 */
gf PQCLEAN_MCELIECE6960119F_SSE_gf_iszero(gf a) {
    uint32_t t = a;
...
gf PQCLEAN_MCELIECE6960119F_SSE_gf_mul(gf in0, gf in1) {
    int i;
...
/* return num/den */
gf PQCLEAN_MCELIECE6960119F_SSE_gf_frac(gf den, gf num) {
    gf tmp_11;
    gf tmp_1111;
    gf out;
...
/* return 1/den */
gf PQCLEAN_MCELIECE6960119F_SSE_gf_inv(gf in) {
    return PQCLEAN_MCELIECE6960119F_SSE_gf_frac(in, ((gf) 1));
}

/* multiplication in GF((2^m)^t) */
void PQCLEAN_MCELIECE6960119F_SSE_GF_mul(gf *out, const gf *in0, const gf *in1) {
    int i, j;
    ...
    for (i = 0; i < 119; i++) {
        for (j = 0; j < 119; j++) {
            prod[i + j] ^= PQCLEAN_MCELIECE6960119F_SSE_gf_mul(in0[i], in1[j]);
        }
    }
    ...
    for (i = 236; i >= 119; i--) {
        prod[i - 117] ^= PQCLEAN_MCELIECE6960119F_SSE_gf_mul(prod[i], (gf) 6400);
        prod[i - 119] ^= PQCLEAN_MCELIECE6960119F_SSE_gf_mul(prod[i], (gf) 3134);
    }
