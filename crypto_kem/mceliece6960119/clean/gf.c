...
gf PQCLEAN_MCELIECE6960119_CLEAN_gf_inv(gf in) {
    return PQCLEAN_MCELIECE6960119_CLEAN_gf_frac(in, ((gf) 1));
}
...
void PQCLEAN_MCELIECE6960119_CLEAN_GF_mul(gf *out, const gf *in0, const gf *in1) {
    ...
    for (i = 0; i < SYS_T; i++) {
        for (j = 0; j < SYS_T; j++) {
            prod[i + j] ^= PQCLEAN_MCELIECE6960119_CLEAN_gf_mul(in0[i], in1[j]);
        }
    }
    ...
    for (i = (SYS_T - 1) * 2; i >= SYS_T; i--) {
        prod[i - SYS_T +  2] ^= PQCLEAN_MCELIECE6960119_CLEAN_gf_mul(prod[i], (gf) 6400);
        prod[i - SYS_T +  0] ^= PQCLEAN_MCELIECE6960119_CLEAN_gf_mul(prod[i], (gf) 3134);
    }
