...
int PQCLEAN_MCELIECE6960119_SSE_genpoly_gen(gf *out, gf *f) {
    ...
    for (j = 2; j <= SYS_T; j++) {
        PQCLEAN_MCELIECE6960119_SSE_GF_mul(mat[j], mat[j - 1], f);
    }
    ...
    for (j = 0; j < SYS_T; j++) {
        for (k = j + 1; k < SYS_T; k++) {
            mask = PQCLEAN_MCELIECE6960119_SSE_gf_iszero(mat[ j ][ j ]);
        ...
        inv = PQCLEAN_MCELIECE6960119_SSE_gf_inv(mat[j][j]);

        for (c = j; c < SYS_T + 1; c++) {
            mat[ c ][ j ] = PQCLEAN_MCELIECE6960119_SSE_gf_mul(mat[ c ][ j ], inv) ;
        }
        ...
        for (k = 0; k < SYS_T; k++) {
            if (k != j) {
                ...
                for (c = j; c < SYS_T + 1; c++) {
                    mat[ c ][ k ] ^= PQCLEAN_MCELIECE6960119_SSE_gf_mul(mat[ c ][ j ], t);
                }
            }
        }
    }
