...
extern gf PQCLEAN_MCELIECE6960119_AVX_vec_reduce_asm(vec128 *);
extern void PQCLEAN_MCELIECE6960119_AVX_update_asm(void *, gf, int);
...
static inline void vec128_cmov(vec128 out[][2], uint16_t mask) {
    ...
    vec128 m0 = PQCLEAN_MCELIECE6960119_AVX_vec128_set1_16b( mask);
    vec128 m1 = PQCLEAN_MCELIECE6960119_AVX_vec128_set1_16b(~mask);

    for (i = 0; i < GFBITS; i++) {
        v0 = PQCLEAN_MCELIECE6960119_AVX_vec128_and(out[i][1], m0);
        v1 = PQCLEAN_MCELIECE6960119_AVX_vec128_and(out[i][0], m1);
        out[i][0] = PQCLEAN_MCELIECE6960119_AVX_vec128_or(v0, v1);
    }
}
...
static inline void interleave(vec256 *in, int idx0, int idx1, vec256 *mask, int b) {
    ...
    x = PQCLEAN_MCELIECE6960119_AVX_vec256_or(PQCLEAN_MCELIECE6960119_AVX_vec256_and(in[idx0], mask[0]),
            PQCLEAN_MCELIECE6960119_AVX_vec256_sll_4x(PQCLEAN_MCELIECE6960119_AVX_vec256_and(in[idx1], mask[0]), s));

    y = PQCLEAN_MCELIECE6960119_AVX_vec256_or(PQCLEAN_MCELIECE6960119_AVX_vec256_srl_4x(PQCLEAN_MCELIECE6960119_AVX_vec256_and(in[idx0], mask[1]), s),
            PQCLEAN_MCELIECE6960119_AVX_vec256_and(in[idx1], mask[1]));
...
static inline void get_coefs(gf *out, vec256 *in) {
    ...
    for (i = 13; i < 16; i++) {
        buf[i] = PQCLEAN_MCELIECE6960119_AVX_vec256_setzero();
    }

    mask[0][0] = PQCLEAN_MCELIECE6960119_AVX_vec256_set1_16b(0x5555);
    mask[0][1] = PQCLEAN_MCELIECE6960119_AVX_vec256_set1_16b(0xAAAA);
    mask[1][0] = PQCLEAN_MCELIECE6960119_AVX_vec256_set1_16b(0x3333);
    mask[1][1] = PQCLEAN_MCELIECE6960119_AVX_vec256_set1_16b(0xCCCC);
    mask[2][0] = PQCLEAN_MCELIECE6960119_AVX_vec256_set1_16b(0x0F0F);
    mask[2][1] = PQCLEAN_MCELIECE6960119_AVX_vec256_set1_16b(0xF0F0);
    mask[3][0] = PQCLEAN_MCELIECE6960119_AVX_vec256_set1_16b(0x00FF);
    mask[3][1] = PQCLEAN_MCELIECE6960119_AVX_vec256_set1_16b(0xFF00);
    ...
    for (i = 0; i < 16; i++) {
        for (k = 0; k <  4; k++) {
            out[ (4 * 0 + k) * 16 + i ] = (PQCLEAN_MCELIECE6960119_AVX_vec256_extract(buf[i], 0) >> (k * 16)) & GFMASK;
            out[ (4 * 1 + k) * 16 + i ] = (PQCLEAN_MCELIECE6960119_AVX_vec256_extract(buf[i], 1) >> (k * 16)) & GFMASK;
            out[ (4 * 2 + k) * 16 + i ] = (PQCLEAN_MCELIECE6960119_AVX_vec256_extract(buf[i], 2) >> (k * 16)) & GFMASK;
            out[ (4 * 3 + k) * 16 + i ] = (PQCLEAN_MCELIECE6960119_AVX_vec256_extract(buf[i], 3) >> (k * 16)) & GFMASK;
        }
    }
}
...
void PQCLEAN_MCELIECE6960119_AVX_bm(vec128 *out, vec256 *in) {
    ...
    for (i = 0; i < GFBITS; i++) {
        interval[i] = PQCLEAN_MCELIECE6960119_AVX_vec128_setzero();
    }
