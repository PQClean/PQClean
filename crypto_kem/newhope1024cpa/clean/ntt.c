...
void PQCLEAN_NEWHOPE1024CPA_CLEAN_mul_coefficients(uint16_t *poly, const uint16_t *factors) {
    ...
    for (i = 0; i < NEWHOPE_N; i++) {
        poly[i] = PQCLEAN_NEWHOPE1024CPA_CLEAN_montgomery_reduce((poly[i] * factors[i]));
    }
}
...
**************************************************/
PQCLEAN_NEWHOPE1024CPA_CLEAN_ntt(uint16_t *a, const uint16_t *omega) {
    ...
    for (i = 0; i < 10; i += 2) {
        ...
        for (start = 0; start < distance; start++) {
            ...
            for (j = start; j < NEWHOPE_N - 1; j += 2 * distance) {
                ...
                a[j + distance] = PQCLEAN_NEWHOPE1024CPA_CLEAN_montgomery_reduce((W * ((uint32_t)temp + 3 * NEWHOPE_Q - a[j + distance])));
            }
        }
