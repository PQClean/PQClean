                                       ...
                                       uint32 mlen, const uint8 *pk)
{
    ...
    if ((FALSE == bStopFunc) && (FsmSw_Falcon_modq_decode(h, 9, &pk[1], FSMSW_FALCON512_CRYPTO_PUBLICKEYBYTES - 1u) != FSMSW_FALCON512_CRYPTO_PUBLICKEYBYTES - 1u))
    {
        retVal = -1;
        bStopFunc = TRUE;
    }
