                                      ...
                                      const uint8 *sk)
{
    ...
    if (0 == FsmSw_Falcon_complete_private(G, f, g, F, 10, tmp2_1024.b))
    {
        bStopFunc = TRUE;
    }
    ...
    if (bStopFunc == FALSE)
    {
        ...
        FsmSw_Falcon_hash_to_point_ct(&sc, r_1024.hm, 10, tmp2_1024.b);
        ...
        for (uint32 i = 0; i < 0xFFFFFFFFu; i++)
        {
            ...
            v = FsmSw_Falcon_comp_encode(sigbuf, *sigbuflen, r_1024.sig, 10u);
            if (v != 0u)
            {
                *sigbuflen = v;
                retVal = 0;
                                        ...
                                        uint32 mlen, const uint8 *pk)
{
    ...
    if (FALSE == bStopFunc)
    {
        ...
        if (FALSE == bStopFunc)
        {
            ...
            FsmSw_Fips202_shake256_inc_absorb(&sc, m, mlen);
            FsmSw_Fips202_shake256_inc_finalize(&sc);
            FsmSw_Falcon_hash_to_point_ct(&sc, hm, 10u, tmp3_1024.b);

            /* Verify signature. */
            if (0 == FsmSw_Falcon_verify_raw(hm, sig, h, 10u, tmp3_1024.b))
            {
                retVal = -1;
            }
        }
