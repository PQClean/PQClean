...
static uint32 load_bigendian_32(const uint8 *x);
...
static uint32 load_bigendian_32(const uint8 *x)
{
    return (uint32)(x[3]) | (((uint32)(x[2])) << 8) |
           (((uint32)(x[1])) << 16) | (((uint32)(x[0])) << 24);
}

/***********************************************************************************************************************
* Name:        load_bigendian_64
*
* Description: t.b.d.
*
...
static uint64 load_bigendian_64(const uint8 *x)
{
    return (uint64)(x[7]) | (((uint64)(x[6])) << 8) |
           (((uint64)(x[5])) << 16) | (((uint64)(x[4])) << 24) |
           (((uint64)(x[3])) << 32) | (((uint64)(x[2])) << 40) |
           (((uint64)(x[1])) << 48) | (((uint64)(x[0])) << 56);
}

/***********************************************************************************************************************
* Name:        store_bigendian_32
*
* Description: t.b.d.
*
* Arguments:   - uint8  *x: t.b.d.
...
/***********************************************************************************************************************
* Name:        FsmSw_sha224_inc_ctx_clone
*
...
/***********************************************************************************************************************
* Name:        FsmSw_sha256_inc_ctx_clone
*
* Description: Copy the hashing state.
*
...
/***********************************************************************************************************************
* Name:        FsmSw_sha384_inc_ctx_clone
*
* Description: Copy the hashing state.
*
...
/***********************************************************************************************************************
* Name:        FsmSw_sha512_inc_ctx_clone
*
* Description: Copy the hashing state.
*
...
{
    uint64 bytes = load_bigendian_64(&state->ctx[32]);

    (void)crypto_hashblocks_sha256(state->ctx, in, (uint32)(64u * inblocks));
...
{
    uint64 bytes = load_bigendian_64(&state->ctx[64]);

    (void)crypto_hashblocks_sha512(state->ctx, in, (uint32)(128u * inblocks));
...
/***********************************************************************************************************************
* Name:        FsmSw_sha256_inc_finalize
*
* Description: Finalize and obtain the digest. If applicable, this function will free the memory associated with the
*              sha256ctx.
*
...
void FsmSw_sha256_inc_finalize(uint8 *out, sha256ctx *state, const uint8 *in, uint32 inlen)
{
    ...
    if (inlen_temp_input < 56u)
    {
        ...
        (void)crypto_hashblocks_sha256(state->ctx, padded, 64);
    }
    else
    {
        ...
        (void)crypto_hashblocks_sha256(state->ctx, padded, 128);
    }
...
void FsmSw_sha512_inc_finalize(uint8 *out, sha512ctx *state, const uint8 *in, uint32 inlen)
{
    ...
    if (inlen_temp_input < 112u)
    {
        ...
        padded[126] = (uint8) (bytes >> 5);
        padded[127] = (uint8) (bytes << 3);
        (void)crypto_hashblocks_sha512(state->ctx, padded, 128);
    }
    else
    {
        ...
        (void)crypto_hashblocks_sha512(state->ctx, padded, 256);
    }
