...
sint8 FsmSw_Dilithium3_crypto_sign_keypair(uint8 *pk, uint8 *sk)
{
    ...
    polyvecl_D3 mat[K_DILITHIUM3];
    polyvecl_D3 s1, s1hat;
    polyveck_D3 s2, t1, t0;
    ...
    /* Expand matrix */
    FsmSw_Dilithium3_polyvec_matrix_expand(mat, rho);

    /* Sample short vectors s1 and s2 */
    FsmSw_Dilithium3_polyvecl_uniform_eta(&s1, rhoprime, 0u);
    FsmSw_Dilithium3_polyveck_uniform_eta(&s2, rhoprime, L_DILITHIUM3);

    /* Matrix-vector multiplication */
    s1hat = s1;
    FsmSw_Dilithium3_polyvecl_ntt(&s1hat);
    FsmSw_Dilithium3_polyvec_matrix_pointwise_montgomery(&t1, mat, &s1hat);
    FsmSw_Dilithium3_polyveck_reduce(&t1);
    FsmSw_Dilithium3_polyveck_invntt_tomont(&t1);
...
sint8 FsmSw_Dilithium3_crypto_sign_signature(uint8 *sig, uint32 *siglen, const uint8 *m, uint32 mlen, const uint8 *sk)
{
    ...
    uint16 nonce = 0;
    polyvecl_D3 mat[K_DILITHIUM3], s1, y, z;
    polyveck_D3 t0, s2, w1, w0, h;
    poly_D3 cp;
    ...
    FsmSw_Dilithium3_unpack_sk(rho, tr, key, &t0, &s1, &s2, sk);
    ...
    while(TRUE == loop)
    {
        ...
        FsmSw_Dilithium3_polyvecl_uniform_gamma1(&y, rhoprime, nonce++);

        /* Matrix-vector multiplication */
        z = y;
        FsmSw_Dilithium3_polyvecl_ntt(&z);
        FsmSw_Dilithium3_polyvec_matrix_pointwise_montgomery(&w1, mat, &z);
        FsmSw_Dilithium3_polyveck_reduce(&w1);
        FsmSw_Dilithium3_polyveck_invntt_tomont(&w1);

        /* Decompose w and call the random oracle */
        FsmSw_Dilithium3_polyveck_caddq(&w1);
        FsmSw_Dilithium3_polyveck_decompose(&w1, &w0, &w1);
        FsmSw_Dilithium3_polyveck_pack_w1(sig, &w1);

        FsmSw_Fips202_shake256_inc_init(&state);
        FsmSw_Fips202_shake256_inc_absorb(&state, mu, CRHBYTES_DILITHIUM);
        FsmSw_Fips202_shake256_inc_absorb(&state, sig, K_DILITHIUM3 * POLYW1_PACKEDBYTES_DILITHIUM3);
        FsmSw_Fips202_shake256_inc_finalize(&state);
        FsmSw_Fips202_shake256_inc_squeeze(sig, CTILDEBYTES_DILITHIUM3, &state);
        FsmSw_Dilithium3_poly_challenge(&cp, sig); /* uses only the first SEEDBYTES bytes of sig */
        FsmSw_Dilithium3_poly_ntt(&cp);

        /* Compute z, reject if it reveals secret */
        FsmSw_Dilithium3_polyvecl_pointwise_poly_montgomery(&z, &cp, &s1);
        FsmSw_Dilithium3_polyvecl_invntt_tomont(&z);
        ...
        FsmSw_Dilithium3_polyveck_pointwise_poly_montgomery(&h, &cp, &s2);
        FsmSw_Dilithium3_polyveck_invntt_tomont(&h);
        ...
        FsmSw_Dilithium3_polyveck_pointwise_poly_montgomery(&h, &cp, &t0);
        FsmSw_Dilithium3_polyveck_invntt_tomont(&h);
        ...
        n = FsmSw_Dilithium3_polyveck_make_hint(&h, &w0, &w1);
    ...
    /* Write signature */
    FsmSw_Dilithium3_pack_sig(sig, sig, &z, &h);
...
{
    uint16 i;
    uint8 buf[K_DILITHIUM3 * POLYW1_PACKEDBYTES_DILITHIUM3];
    uint8 rho[SEEDBYTES_DILITHIUM];
    uint8 mu[CRHBYTES_DILITHIUM];
    uint8 c[CTILDEBYTES_DILITHIUM3];
    uint8 c2[CTILDEBYTES_DILITHIUM3];
    poly_D3 cp;
    polyvecl_D3 mat[K_DILITHIUM3], z;
    polyveck_D3 t1, w1, h;
    shake256incctx state;
    ...
    FsmSw_Dilithium3_unpack_pk(rho, &t1, pk);
    if (0 < FsmSw_Dilithium3_unpack_sig(c, &z, &h, sig))
    {
        return -1;
    }
    if (0 < FsmSw_Dilithium3_polyvecl_chknorm(&z, (sint32)(GAMMA1_DILITHIUM3 - BETA_DILITHIUM3)))
    ...
    FsmSw_Dilithium3_polyveck_pointwise_poly_montgomery(&t1, &cp, &t1);
    ...
    /* Reconstruct w1 */
    FsmSw_Dilithium3_polyveck_caddq(&w1);
    FsmSw_Dilithium3_polyveck_use_hint(&w1, &w1, &h);
    FsmSw_Dilithium3_polyveck_pack_w1(buf, &w1);

    /* Call random oracle and verify FsmSw_Dilithium3_challenge */
    FsmSw_Fips202_shake256_inc_init(&state);
    FsmSw_Fips202_shake256_inc_absorb(&state, mu, CRHBYTES_DILITHIUM);
    FsmSw_Fips202_shake256_inc_absorb(&state, buf, K_DILITHIUM3 * POLYW1_PACKEDBYTES_DILITHIUM3);
    FsmSw_Fips202_shake256_inc_finalize(&state);
    FsmSw_Fips202_shake256_inc_squeeze(c2, CTILDEBYTES_DILITHIUM3, &state);
...
sint8 FsmSw_Dilithium3_crypto_sign_open(uint8 *m, uint32 *mlen, const uint8 *sm, uint32 smlen, const uint8 *pk)
{
    ...
    if (smlen >= FSMSW_DILITHIUM3_CRYPTO_BYTES)
    {
        *mlen = smlen - FSMSW_DILITHIUM3_CRYPTO_BYTES;
        if (0 == FsmSw_Dilithium3_crypto_sign_verify(sm, FSMSW_DILITHIUM3_CRYPTO_BYTES,
                                                     &sm[FSMSW_DILITHIUM3_CRYPTO_BYTES], *mlen, pk))
        {
