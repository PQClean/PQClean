                              ...
                              const polyveck_D3 *t1)
{
    ...
    {
        FsmSw_Dilithium3_polyt1_pack(&pk_temp[i * POLYT1_PACKEDBYTES_DILITHIUM], &t1->vec[i]);
    }
}
...
void FsmSw_Dilithium3_unpack_pk(uint8 rho[SEEDBYTES_DILITHIUM],
    ...
    for (i = 0; i < K_DILITHIUM3; ++i)
    {
        FsmSw_Dilithium3_polyt1_unpack(&t1->vec[i], &pk_temp[i * POLYT1_PACKEDBYTES_DILITHIUM]);
    }
}
                              ...
                              const polyveck_D3 *s2)
{
    ...
    {
        FsmSw_Dilithium3_polyeta_pack(&sk_temp[i * POLYETA_PACKEDBYTES_DILITHIUM3], &s1->vec[i]);
    }
    sk_temp = &sk_temp[L_DILITHIUM3 * POLYETA_PACKEDBYTES_DILITHIUM3];

    for (i = 0; i < K_DILITHIUM3; ++i)
    {
        FsmSw_Dilithium3_polyeta_pack(&sk_temp[i * POLYETA_PACKEDBYTES_DILITHIUM3], &s2->vec[i]);
    }
    sk_temp = &sk_temp[K_DILITHIUM3 * POLYETA_PACKEDBYTES_DILITHIUM3];

    for (i = 0; i < K_DILITHIUM3; ++i)
    {
        FsmSw_Dilithium3_polyt0_pack(&sk_temp[i * POLYT0_PACKEDBYTES_DILITHIUM], &t0->vec[i]);
    }
}
                                ...
                                const uint8 sk[FSMSW_DILITHIUM3_CRYPTO_SECRETKEYBYTES])
{
    ...
    for (i = 0; i < L_DILITHIUM3; ++i)
    {
        FsmSw_Dilithium3_polyeta_unpack(&s1->vec[i], &sk_temp[i * POLYETA_PACKEDBYTES_DILITHIUM3]);
    }
    sk_temp = &sk_temp[L_DILITHIUM3 * POLYETA_PACKEDBYTES_DILITHIUM3];

    for (i = 0; i < K_DILITHIUM3; ++i)
    {
        FsmSw_Dilithium3_polyeta_unpack(&s2->vec[i], &sk_temp[i * POLYETA_PACKEDBYTES_DILITHIUM3]);
    }
    sk_temp = &sk_temp[K_DILITHIUM3 * POLYETA_PACKEDBYTES_DILITHIUM3];

    for (i = 0; i < K_DILITHIUM3; ++i)
    {
        FsmSw_Dilithium3_polyt0_unpack(&t0->vec[i], &sk_temp[i * POLYT0_PACKEDBYTES_DILITHIUM]);
    }
}
                               ...
                               const polyveck_D3 *h)
{
    ...
    for (i = 0; i < L_DILITHIUM3; ++i)
    {
        FsmSw_Dilithium3_polyz_pack(&sig_temp[i * POLYZ_PACKEDBYTES_DILITHIUM3], &z->vec[i]);
    }
                                ...
                                const uint8 sig[FSMSW_DILITHIUM3_CRYPTO_BYTES])
{
    ...
    for (i = 0; i < L_DILITHIUM3; ++i)
    {
        FsmSw_Dilithium3_polyz_unpack(&z->vec[i], &sig_temp[i * POLYZ_PACKEDBYTES_DILITHIUM3]);
    }
    sig_temp = &sig_temp[L_DILITHIUM3 * POLYZ_PACKEDBYTES_DILITHIUM3];
