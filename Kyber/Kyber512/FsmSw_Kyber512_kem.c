...
uint8 FsmSw_Kyber512_crypto_kem_keypair(uint8 *pk, uint8 *sk)
{
    ...
    hash_h(&sk[KYBER512_SECRETKEYBYTES - (2u * KYBER_SYMBYTES)], pk, KYBER512_PUBLICKEYBYTES);
...
uint8 FsmSw_Kyber512_crypto_kem_enc(uint8 *ct, uint8 *ss, const uint8 *pk)
{
    ...
    hash_h(&buf[KYBER_SYMBYTES], pk, KYBER512_PUBLICKEYBYTES);
    ...
    hash_h(&kr[KYBER_SYMBYTES], ct, KYBER512_CIPHERTEXTBYTES);
